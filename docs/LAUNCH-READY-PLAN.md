...
- Terminal: **Native** Local PowerShell first paint — ConPTY → xterm.js in the Remote bottom panel (not WinUI TermControl overlay, not detached `wt`); geometry fills hole; ConPTY buffer+ACK  
...
| Terminal Native ConPTY→xterm first-open paint + geometry | Code landed (TermControl overlay retired) | ☐ |
