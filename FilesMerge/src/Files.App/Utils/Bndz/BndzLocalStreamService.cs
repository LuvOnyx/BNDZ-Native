// Copyright (c) BNDZ — FilesMerge pane WebView local streaming (mirrors BNDZBackend LocalStreamService).
...
/// <summary>
/// Serves local files to Files-hosted React panes via <c>bndz-stream://</c> (byte-range aware).
/// Must NOT use the <c>bndz.local</c> virtual-host folder mapping — WebResourceRequested does not fire there.
/// </summary>
internal static class BndzLocalStreamService
{
	...
	public static string ParseLocalStreamPath(string requestUri)
	{
		...
		{
			string withoutScheme = requestUri.Substring(CustomScheme.Length + 1);
	...
	{
		string ext = SystemIO.Path.GetExtension(path).ToLowerInvariant();
		return ext switch
		{
		...
		string localPath)
	{
		...
		{
			e.Response = env.CreateWebResourceResponse(EmptyRas(), 204, "No Content",
		...
		if (string.IsNullOrEmpty(localPath) || !SystemIO.File.Exists(localPath))
		{
			e.Response = env.CreateWebResourceResponse(EmptyRas(), 404, "Not Found", "Content-Type: text/plain\r\nAccess-Control-Allow-Origin: *");
			return;
		}

		var fileInfo = new SystemIO.FileInfo(localPath);
		...
		{
			var stream = new SystemIO.FileStream(localPath, SystemIO.FileMode.Open, SystemIO.FileAccess.Read, SystemIO.FileShare.ReadWrite | SystemIO.FileShare.Delete);
			string responseHeaders =
				$"Content-Type: {contentType}\r\n" +
				$"Content-Length: {fileLength}\r\n" +
				"Accept-Ranges: bytes\r\n" +
				"Access-Control-Allow-Origin: *";
			e.Response = env.CreateWebResourceResponse(stream.AsRandomAccessStream(), 200, "OK", responseHeaders);
			return;
		}
		...
		if (!match.Success || fileLength <= 0)
		{
			if (fileLength <= 0)
			{
				e.Response = env.CreateWebResourceResponse(EmptyRas(), 416, "Range Not Satisfiable",
					$"Content-Range: bytes */0\r\nAccept-Ranges: bytes\r\nAccess-Control-Allow-Origin: *");
				return;
			}

			var stream = new SystemIO.FileStream(localPath, SystemIO.FileMode.Open, SystemIO.FileAccess.Read, SystemIO.FileShare.ReadWrite | SystemIO.FileShare.Delete);
			e.Response = env.CreateWebResourceResponse(stream.AsRandomAccessStream(), 200, "OK",
				$"Content-Type: {contentType}\r\nContent-Length: {fileLength}\r\nAccept-Ranges: bytes\r\nAccess-Control-Allow-Origin: *");
			return;
		}
		...
		var partialStream = new BndzPartialFileStream(localPath, start, length);
		...
		e.Response = env.CreateWebResourceResponse(partialStream.AsRandomAccessStream(), 206, "Partial Content", partialHeaders);
	}
...
/// <summary>Seekable read-only window over a file byte range (WebView2 rewinds response streams).</summary>
internal sealed class BndzPartialFileStream : Stream
{
	private readonly SystemIO.FileStream _inner;
	private readonly long _fileOffset;
	private readonly long _length;
	private long _position;

	public BndzPartialFileStream(string path, long fileOffset, long length)
	{
		_inner = new SystemIO.FileStream(path, SystemIO.FileMode.Open, SystemIO.FileAccess.Read, SystemIO.FileShare.ReadWrite | SystemIO.FileShare.Delete);
