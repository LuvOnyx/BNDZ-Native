...
{
    public sealed class BndzIpcHost : System.Windows.Window
    {
        ...
        private sealed class HeadlessWebViewStub
        {
            ...
            public Microsoft.Web.WebView2.Core.CoreWebView2? CoreWebView2 => null;
            public System.Windows.Point PointFromScreen(System.Windows.Point p) => p;
            public System.Windows.Point PointToScreen(System.Windows.Point p) => p;
            public System.Windows.Media.GeneralTransform TransformToAncestor(System.Windows.DependencyObject _) =>
...
#pragma warning restore CS0067
            public Task EnsureCoreWebView2Async(Microsoft.Web.WebView2.Core.CoreWebView2Environment? _ = null) => Task.CompletedTask;
        }
        private readonly HeadlessWebViewStub MainWebView = new();

        private static void InvokeInline(Action action) { try { action(); } catch { } }
        private static T InvokeInline<T>(Func<T> func) { try { return func(); } catch { return default!; } }
        private static System.Windows.Threading.DispatcherOperation InvokeInlineAsync(Action action)
        {
            action();
            return System.Windows.Threading.Dispatcher.CurrentDispatcher.BeginInvoke(new Action(() => { }), System.Windows.Threading.DispatcherPriority.Background);
        }
        private static System.Windows.Threading.DispatcherOperation BeginInvokeInline(Delegate d, params object[] args)
        {
            try { d.DynamicInvoke(args); } catch { }
            return System.Windows.Threading.Dispatcher.CurrentDispatcher.BeginInvoke(new Action(() => { }), System.Windows.Threading.DispatcherPriority.Background);
        }
        private static System.Windows.Threading.DispatcherOperation BeginInvokeInline(System.Windows.Threading.DispatcherPriority priority, Delegate d)
        {
            try { d.DynamicInvoke(); } catch { }
            return System.Windows.Threading.Dispatcher.CurrentDispatcher.BeginInvoke(new Action(() => { }), System.Windows.Threading.DispatcherPriority.Background);
        }
        ...
        /// <summary>WinUI shell HWND for shell verbs / dialogs / Automation host.</summary>
        public void SetHostWindowHandle(IntPtr hwnd)
        {
        ...
        public void SetHostTrayActions(Action? hideToTray, Action? restoreFromTray)
        {
        ...
        private bool HasLiveUiTransport() => _pushWebMessage != null || MainWebView.CoreWebView2 != null;


        public BndzIpcHost(FileManagementService fileService, AiAssistantService aiService, LocalAiService localAi, ShellIntegrationService shellIntegrationService, Action<string>? pushWebMessage = null, IntPtr hostWindowHandle = default)
        {
        ...
        private void ApplyNativeShellMode_Legacy()
        {
            ...
            if (NativeShellChrome != null)
            {
                ...
                NativeShellChrome?.MouseLeftButtonDown += (_, e) =>
                {
                    ...
                    {
                        ReleaseCapture();
                        SendMessage((_hostWindowHandle != IntPtr.Zero ? _hostWindowHandle : new System.Windows.Interop.WindowInteropHelper(this).Handle), WM_NCLBUTTONDOWN, (IntPtr)HTCAPTION, IntPtr.Zero);
                    }
                };
            }
        ...
        /// </summary>
        private void ApplyEmbeddedMode_Legacy()
        {
            Title = "BNDZ Embedded";
            if (NativeShellChrome != null)
        ...
        /// </summary>
        private void ApplyBackendHostMode_Legacy()
        {
        ...
        protected override void OnSourceInitialized(EventArgs e)
        {
            // headless
            // headless: no window icon
            var source = System.Windows.Interop.HwndSource.FromHwnd((_hostWindowHandle != IntPtr.Zero ? _hostWindowHandle : new System.Windows.Interop.WindowInteropHelper(this).Handle));
            ...
            bool skipGlyphs = false)
        {
            ...
            if (skipGlyphs && page.Count > 0)
            {
                _ = Task.Run(() =>
                {
                    try
                    {
                        var lateGlyphs = ShellGlyphMapService.Instance.BuildListingGlyphMap(page);
                        if (lateGlyphs.Count == 0) return;
                        _ = PostToUiAsync(() =>
                        {
                            var response = new
                            {
        ...
        private async Task ProcessIncomingIpcMessageAsync(string messageStr)
        {
            try
            {
                ...
                else if (type == "SET_SHELL_CLIPBOARD")
                {
                    ...
                    try
                    {
                        ok = InvokeInline(() => _shellContextMenuService.TrySetShellClipboard(paths, cut));
                    }
                    catch (Exception ex)
                    {
                ...
                else if (type == "GET_SHELL_CLIPBOARD")
                {
                    ...
                    try
                    {
                        var (paths, cut, ok) = InvokeInline(() => _shellContextMenuService.TryGetShellClipboard());
                ...
                else if (type == "CLEAR_SHELL_CLIPBOARD")
                {
                    ...
                    try
                    {
                        ok = InvokeInline(() => _shellContextMenuService.TryClearShellClipboard());
                    }
                    catch (Exception ex)
                    {
                    ...
                    InvokeInline(() =>
                    {
                        switch (action.ToLowerInvariant())
                        {
                            case "tray":
                                ...
                                if (_hostHideToTrayAction != null)
                                    _hostHideToTrayAction();
                                else
                ...
                else if (type == "TRAY_RESTORE")
                {
                    InvokeInline(() =>
                    {
                        if (_hostRestoreFromTrayAction != null)
                            _hostRestoreFromTrayAction();
                        else
                            _trayService?.RestoreMainWindow();
                    });
                }
                else if (type == "REQUEST_CLOSE")
                {
                    ...
                    {
                        InvokeInline(() => RequestCloseFromUI(source));
                    }
                }
                ...
                else if (type == "RUN_AUTOMATION_GRAPH")
                {
                    ...
                    _ = Task.Run(() =>
                    {
                        _automationRunnerDeps.HostWindow = (_hostWindowHandle != IntPtr.Zero ? _hostWindowHandle : new System.Windows.Interop.WindowInteropHelper(this).Handle);
        ...
        private bool TryHandleFastIpc(string type, string? idProp, JsonElement root)
        {
            ...
            if (type == "GET_DRIVES")
            {
                ...
                try
                {
                    if (root.TryGetProperty("payload", out var drivePayload)
                        && drivePayload.ValueKind == JsonValueKind.Object
                        && drivePayload.TryGetProperty("force", out var forceEl)
                        && (forceEl.ValueKind == JsonValueKind.True
                            || (forceEl.ValueKind == JsonValueKind.String
                                && string.Equals(forceEl.GetString(), "true", StringComparison.OrdinalIgnoreCase))))
                    {
                        forceRefresh = true;
                    }
                }
                ...
                _ = Task.Run(() =>
                {
                    try
                    {
                        var drives = _cloudStorageService.GetAnnotatedDrives(forceRefresh);
