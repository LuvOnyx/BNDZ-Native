...
try {
    if ($Configuration -eq "Release") {
        ...
        if (-not $secret -or $secret -eq "BNDZ-36-Commercial-Key-Seed-CHANGE-ME") {
            throw "BNDZ_LICENSE_SECRET must be set to a non-placeholder value before Release builds. This secret is embedded into BNDZ.exe for offline serial validation."
        }
        $releaseLicPath = Join-Path $Backend "Services\LicenseSecretEmbedded.Release.cs"
        $escaped = $secret.Replace('\', '\\').Replace('"', '\"')
        $licSource = @"
// AUTO-GENERATED by scripts/build-release.ps1 — do not commit.
...
"@
        Set-Content -Path $releaseLicPath -Value $licSource -Encoding UTF8
        Write-Host "==> Embedded retail license secret for Release build" -ForegroundColor Yellow

        $tokenSecret = $env:BNDZ_TOKEN_HMAC_SECRET
        if (-not $tokenSecret) {
            throw "BNDZ_TOKEN_HMAC_SECRET must be set before Release builds (must match Cloudflare Worker TOKEN_HMAC_SECRET)."
        }
        $releaseTokPath = Join-Path $Backend "Services\LicenseTokenSecretEmbedded.Release.cs"
...
"@
        Set-Content -Path $releaseTokPath -Value $tokSource -Encoding UTF8
        Write-Host "==> Embedded retail token HMAC secret for Release build" -ForegroundColor Yellow
    } elseif ($BuildInstaller -and -not $env:BNDZ_LICENSE_SECRET) {
