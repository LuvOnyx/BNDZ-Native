...
REM Prefer newest bin\x64\Debug BNDZ.exe (legacy BNDZShell.exe still accepted).
set "EXE="
for /f "delims=" %%F in ('dir /b /s /a:-d "BNDZShell\src\BNDZShell.App\bin\x64\Debug\BNDZ.exe" 2^>nul') do (
  set "EXE=%%F"
  goto :found
)
...
for /f "delims=" %%F in ('dir /b /s /a:-d "BNDZShell\src\BNDZShell.App\bin\x64\Release\BNDZ.exe" 2^>nul') do (
  set "EXE=%%F"
  goto :found
)
...
if exist "artifacts\bndzshell-debug\BNDZ.exe" (
