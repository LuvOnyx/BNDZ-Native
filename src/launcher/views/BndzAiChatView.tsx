...
function getMenuPanelStyle(): { className: string; style: React.CSSProperties } {
  ...
  const style: React.CSSProperties = isNativeLiquidGlass
    ? {
        ...
        WebkitBackdropFilter: 'blur(44px) saturate(155%)' as React.CSSProperties['WebkitBackdropFilter'],
    ...
    ? {
        ...
        WebkitBackdropFilter: 'blur(128px) saturate(195%) contrast(107%) brightness(1.03)' as React.CSSProperties['WebkitBackdropFilter'],
...
const BndzBadge: React.FC = () => (
  <span
    className="inline-flex items-center justify-center px-1.5 h-4 rounded-sm text-[7px] font-bold tracking-wider leading-none select-none"
    style={{
...
const BndzAiChatView: React.FC<BndzAiChatViewProps> = ({
  aiQuery,
  setAiQuery,
  messages,
  aiStreaming,
  ...
  return (
    <div className="w-full h-full">
      <div className="glass-effect overflow-hidden h-full flex flex-col relative">
        <div className="flex items-center gap-3 px-4 py-3 border-b border-[var(--ui-divider)]">
          <button
            type="button"
            onClick={exitAiMode}
