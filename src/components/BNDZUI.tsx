...
import WorkIntentSwitcher from './WorkIntentSwitcher';
...
import { isBndzVirtualPath, isBndzHomePath, isBndzCanvasPath, isBndzAutomationPath, isBndzWorkspacePath, isBndzRamPath, isFsDropTargetPath, parseBndzRamZoneId, parseBndzVirtualView, bndzVirtualPath, bndzVirtualLabel, bndzRamVirtualPath, BNDZ_VIEWS_ROOT, BNDZ_HOME, BNDZ_CANVAS, BNDZ_AUTOMATION, BNDZ_RAM_ROOT, BNDZ_PROBLEMS, BNDZ_INBOUND } from '../lib/bndzVirtualViews';
...
import { setPathCacheEntry, setPinnedPathCacheKeys } from '../lib/pathCacheLru';
...
import { resolveFileDragHoverAtPoint, setExternalDragHover, setPointerDragHover, clearPointerDragHover, recordExternalDragHover } from '../lib/fileDragHover';
...
export default function BNDZUI() {
  ...
  const beginMarqueeGesture = React.useCallback((
    ...
    const onPointerUp = () => {
      ...
      if (moved) {
        ...
        ops.scheduleSelectionChrome([], true);
  ...
  const [smartToolsTab, setSmartToolsTab] = useState<'assistant' | 'organize' | 'duplicates' | 'music'>('assistant');
        ...
        if (job.status === 'completed' || job.status === 'failed' || job.status === 'cancelled') {
          if (!completed.has(job.operationId)) {
            ...
            if (job.status === 'failed') {
              ...
              const doneVerb = action === 'delete' || meta?.op === 'delete' ? 'Deleted'
  ...
  useEffect(() => {
    ...
    if (!currentDirCount) return;
    // Same scheduler as above when both flags are on — skip duplicate 1.6s kick.
  ...
  const navigationDrives = useMemo(
    () => drives.filter(d => !isCloudOwnedDrive(d)),
    [drives],
  );
  ...
  }, [config.pinnedFavorites, config.hiddenRapidAccess, config.selectConfig10, config.rapidAccessOrder, shortcuts, windowsUsername, galleryShortcut]);
  ...
  const copyTextToClipboard = async (text: string, okMsg: string) => {
    try {
      ...
      setToastMessage(okMsg);
    } catch {
      setToastMessage('Could not copy to clipboard.', 'warning');
    }
  };
  ...
  // Host: AllowExternalDrop toggled by OLE session — WPF PreviewDrop + NavigationStarting file: fallback.
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      const listEl = document.querySelector(
        `[data-list-body][data-list-pane-id="${activePaneId}"]`,
      ) as HTMLElement | null;
  ...
  const handleApplyRename = (operations: RenameOperation[]) => {
    ...
    const rootDriveEntities = navigationDrives.map(d => {
        ...
        return {
            ...
            name: d.label ? `${d.label} (${letter})` : formatDriveRootLabel(normalizedName),
            type: "directory",
            path: normalizedName,
            size: d.totalSpace,
            tags: [],
  ...
  const renderPane = (pane: PaneState, index: number) => {
    ...
    const renderDetailColumn = (
      ...
      switch (colId) {
        ...
        case 'tags':
          return (
            <div key={colId} className="bndz-list-select-cell px-2 flex gap-1 h-full items-center overflow-hidden min-w-0">
              {settingsRt.list.showTags && [...new Set(entityTags)].map(t => (
    ...
    return (
      ...
      >
        ...
        <div className={`flex ${config.applyColors ? '' : 'bg-[#1a1a1a]'} border-b border-[#333] items-center px-1 py-[2px] shrink-0 ${isDualPane && !isActive ? 'opacity-90' : ''}`}
                    ...
                    onKeyDown={async e => {
                        ...
                        if (e.key === 'Enter') {
                            ...
                            import('../lib/ipcBridge').then(async ({ IPC }) => {
                              ...
                              const parsedPath = (await resolveUserPathToPane(raw, expand))
                                || parseUserPathToPane(raw);
                              if (!parsedPath) {
                                setEditingAddressBarPaneId(null);
                                return;
                              }
            ...
            <div className="shrink-0 mx-1 flex items-center gap-1.5 w-[230px]">
        ...
        <div
           className={`fs-list-header bndz-list-header-bar flex text-[11px] shrink-0 select-none ${!settingsRt.list.verticalGridLines ? 'bndz-list-header-bar--no-grid' : ''}`}
           ...
           onPointerDownCapture={(e) => {
              ...
              const syncDragModifiers = (e: { ctrlKey?: boolean; metaKey?: boolean; altKey?: boolean }) => {
                ...
                if (listGestureRef.current?.mode === 'drag') {
                  ...
                  setListDragGhost(g => (g ? { ...g, copy, dropHint } : g));
                  if (fluidDragEnabled) updateFluidDragMeta({ copy, dropHint });
                }
              };
              ...
              const onMove = (ev: PointerEvent) => {
                ...
                if (listGestureRef.current.mode === 'pending') {
                  ...
                  const marqueeIntent = !preferDrag && (g.shiftKey || (dx > 10 && dx > dy * 1.5));
                  ...
                  return (
                    <div
                      className={`z-10 flex items-center gap-2 px-2 text-[10px] font-bold uppercase tracking-wider text-[#99c9f0] bg-[#252526] border-y border-[#454545] ${
                        stickyOn ? '' : 'sticky top-0'
                      }`}
                      style={{ height: detailsRowHeight, boxSizing: 'border-box' }}
                    >
                              ...
                              <>
                               <div className="bndz-list-select-cell flex flex-col items-center w-full h-full relative overflow-hidden">
                                  ...
                                  {isDir && colorFilterResult?.folderIcon ? (
                                    <FolderColorIcon folderIconId={colorFilterResult.folderIcon} size={gridMetrics.icon} title={entity.name} />
                                  ) : (
  ...
  return (
    ...
    >
                 ...
                 <MenubarPortalMenu open={openMenuId === 'Go'} anchorEl={menubarAnchors.current['Go']} minWidth={260}>
                    ...
                    <MenubarSubmenu label="Drives">
                      {(drives || []).length === 0 ? (
                        ...
                        drives.map((d: any) => (
                          ...
                          >
                            {formatDriveRootLabel(d.name)}{d.label ? ` — ${d.label}` : ''}
                          </div>
                        ))
                      )}
                    </MenubarSubmenu>
                    ...
                    <MenubarSubmenu label="Hotlist (Rapid access)">
                      {rapidAccessItems.length === 0 ? (
                        ...
                        rapidAccessItems.slice(0, 16).map((s) => (
                          <div key={s.path} className="px-3 py-1 hover:bg-[#007acc] cursor-pointer text-sm text-gray-200 truncate max-w-[320px]" title={s.path} onMouseDown={menuAct(() => setCurrentPath(s.path))}>
                            {s.name || s.path}
                          </div>
                        ))
                      )}
                    ...
                    <MenubarSubmenu label="Tablist">
                      {currentPane?.tabs?.length ? currentPane.tabs.map((t: any, i: number) => (
                        <div key={t.id || i} className="px-3 py-1 hover:bg-[#007acc] cursor-pointer text-sm text-gray-200 truncate max-w-[320px]" title={t.path} onMouseDown={menuAct(() => {
                          setPanes(prev => prev.map(p => p.id === activePaneId ? { ...p, activeTabIndex: i } : p));
                        })}>
                          {getPaneTabLabel(t.path) || t.path || `Tab ${i + 1}`}
                        </div>
                      )) : (
      ...
      <div data-tutorial="omnibar" className="bndz-chrome-omnibar flex px-2 py-1 items-center border-b border-[#333] shrink-0 gap-2" style={{ background: 'var(--toolbar-bg, var(--bndz-surface-chrome))' }}>
         <WorkIntentSwitcher openBottomPlugin={openBottomPlugin} />
         ...
         >
            ...
            >
               <LeftSidebar
                  ...
                  drivesContent={navigationDrives.length > 0 && navigationDrives.map(drive => (
                     <div
                        key={drive.name}
                  ...
                  >
                    {(() => {
                     ...
                     <div className="flex-1 flex flex-col h-full min-h-0 overflow-hidden bg-[#202020] relative">
                        ...
                        {config.commandDeck !== false && commandDeckSignature.kind !== 'empty' && (
