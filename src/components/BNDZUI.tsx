...
import {
  ...
  DRAG_DELAY_NATIVE, DRAG_DELAY_NATIVE_SELECTED, resetDragInteractionState,
...
import { isVideoExt, isShellActivateExt } from '../lib/mediaTypes';
...
import { cloudBadgeForPath, cloudSidebarStatusLabel, resolveEntityCloudStatus, type CloudProvider } from '../lib/cloudStatus';
...
import { resolveFileDragHoverAtPoint, setExternalDragHover, setPointerDragHover, clearPointerDragHover, clearExternalDragHover, recordExternalDragHover, recallPointerDragHover } from '../lib/fileDragHover';
...
function formatSize(bytes: number | null | undefined) {
  ...
  // Non-breaking space keeps "797.3 MB" on one line next to the size bar.
...
export default function BNDZUI() {
  ...
  const keyboardMap = useMemo(() => {
    // Settings â†’ Custom Keyboard Shortcuts (literal toggle for strong wiring)
  ...
  // Real undo/redo availability for the main toolbar â€” previously always-enabled regardless
  ...
  const marqueeOpsRef = useRef({
    ...
    setFocusedItemId: (_id: string | null) => {},
    setMarqueePreviewIds: (_ids: string[] | null) => {},
    scheduleSelectionChrome: (_ids: string[], _immediate: boolean) => {},
    scheduleQuickActionsBar: (_show: boolean, _immediate?: boolean) => {},
  });

  /** Per-pane DOM elements for the marquee rect â€” updated imperatively, zero React renders. */
  const marqueeRectByPaneRef = useRef<Map<string, HTMLElement>>(new Map());
  /**
   * Live marquee selection preview â€” DOM + renderItem read this without React setState
  ...
  const beginMarqueeGesture = React.useCallback((
    ...
    // Cache list geometry Î“Ã‡Ã¶ avoid getBoundingClientRect every move.
    ...
    const applyMarqueeSelectionPreview = (state: typeof marqueeState) => {
      ...
      if (nowMs - lastMarqueePreviewMs >= 150) {
        lastMarqueePreviewMs = nowMs;
        marqueeOpsRef.current.setMarqueePreviewIds(finalSelected);
      }
      // No React setState / quick-actions during scrub â€” commit on pointerup.
    };
    ...
    const onPointerUp = () => {
      ...
      if (moved) {
        ...
        if (latestSelected.length > 0) ops.setFocusedItemId(latestSelected[0]);
        else ops.setFocusedItemId(null);
        ops.scheduleQuickActionsBar(latestSelected.length > 0, true);
      } else if (!additive) {
        // Plain click on empty canvas / marquee gutter â€” clear selection (Explorer-class).
      ...
      pendingMarqueeState = null;
      marqueeLiveSelectionRef.current = null;
      marqueeOpsRef.current.setMarqueePreviewIds(null);
  ...
  // Launch Ready A1: no sidebar RAM/Ghost tree chrome â€” zones only used to hide mount letters from This PC.
  ...
  useEffect(() => {
    ...
    import('../lib/ipcBridge').then(({ IPC }) => {
      ...
      void pull(false);
      // Forced retry â€” first paint often races DriveInfo; force bypasses warm empty-size cache.
      ...
      if (!netBeh.noNetworkBrowsingAtStartup) {
        ...
        // WSL distro discovery can outlast the first 600ms probe â€” retry like drives.
        window.setTimeout(() => {
          if (cancelled) return;
          IPC.getNetworkLocations({
            assumeMappedReady: !!netBeh.assumeThatMappedNetworkDrivesAreAvailable
              ...
              || !!netBeh.assumeThatServersAreAvailable,
            cacheServers: !!netBeh.cacheNetworkServers || !!config.cacheNetworkServers,
          }).then(n => { if (!cancelled) setNetworkNodes(Array.isArray(n) ? n : []); });
        }, 2800);
      } else if (!cancelled) {
        setNetworkNodes([]);
      }
  ...
  useEffect(() => {
    ...
    updateConfig({
      ...
      // Always restore the canonical bottom plugin height on layout bumps â€”
      // keeping a persisted ~5â€“10% leaves the dock looking collapsed.
  ...
  /** Keep outer columns on the canonical 12 / 72 / 16 split after a layout-version bump. */
  useEffect(() => {
    if ((config.workspaceLayoutVersion ?? 0) !== WORKSPACE_LAYOUT_VERSION) return;
    if (config.previewDockedInWorkspace === true) return;
    const target = {
      sidebar: DEFAULT_OUTER_LAYOUT.sidebar!,
      workspace: DEFAULT_OUTER_LAYOUT.workspace!,
      preview: DEFAULT_OUTER_LAYOUT.preview!,
    };
    const outer = config.workspaceLayoutOuter as Record<string, number> | undefined;
    const drifted =
      !outer
      || Math.abs((outer.sidebar ?? 0) - target.sidebar) > 0.6
      || Math.abs((outer.preview ?? 0) - target.preview) > 0.6;
    if (!drifted) return;
    updateConfig({ workspaceLayoutOuter: target });
    // Only correct once per layout-version bump â€” do not fight later user drag-resizes.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [config.workspaceLayoutVersion, config.previewDockedInWorkspace]);
  ...
  useEffect(() => {
    ...
    if ((config.productDefaultsVersion ?? 0) < 3) {
      ...
      // List names were shipping as muted #e0e0e0 â€” whiten unless the user already customized.
    ...
    /** Old default stretched a single tab across the bar â€” force compact once. */
  ...
  const togglePreviewPanel = () => {
    ...
    // Imperatively restore a usable width â€” collapse leaves size 0 and expand alone
    // can reopen paper-thin if defaultLayout was not remounted.
    if (next) {
      const saved = previewDockedInWorkspace
        ? Number(config.workspaceLayoutMainRow?.preview ?? DEFAULT_OUTER_LAYOUT.preview)
        : Number(config.workspaceLayoutOuter?.preview ?? DEFAULT_OUTER_LAYOUT.preview);
      const pct = Math.max(
        Number.isFinite(saved) && saved > 0 ? saved : DEFAULT_OUTER_LAYOUT.preview!,
        PREVIEW_COLLAPSE_SIZE,
        MIN_PREVIEW_SIZE,
      ...
      requestAnimationFrame(() => {
        try {
          ...
          previewPanelRef.current?.resize(panelPct(pct));
        } catch { /* ignore */ }
        if (!previewDockedInWorkspace && outerGroupRef.current) {
          try {
            const layout = computeVisibleOuterLayout(
              { ...(config.workspaceLayoutOuter || DEFAULT_OUTER_LAYOUT), preview: pct },
              uiRuntime.treePanel,
              true,
            );
            outerGroupRef.current.setLayout(layout);
          } catch { /* ignore */ }
        }
      });
    }
  ...
  /** Updated after pane state â€” workspace tools hide the bottom plugin panel. */
  ...
  const saveOuterLayout = (layout: Record<string, number>) => {
    ...
    // When preview is closed, keep the last open width â€” never persist 0 (paper-thin reopen).
    // While open, never persist below the collapse floor (collapse drag must not poison reopen).
    let previewVal: number;
    if (previewDockedInWorkspace) {
      previewVal = 0;
    } else if (effectivePreviewOpen) {
      const raw = layout.preview ?? outerLayoutLive.preview ?? DEFAULT_OUTER_LAYOUT.preview!;
      if (raw < PREVIEW_COLLAPSE_SIZE) {
        previewVal = config.workspaceLayoutOuter?.preview
          ?? outerLayoutLive.preview
          ?? DEFAULT_OUTER_LAYOUT.preview!;
      } else {
        previewVal = raw;
      }
    ...
    previewVal = Math.max(Number(previewVal) || DEFAULT_OUTER_LAYOUT.preview!, PREVIEW_COLLAPSE_SIZE);
    if (previewDockedInWorkspace) previewVal = 0;
  ...
  const exitBottomImmersive = React.useCallback(() => {
    ...
    motionPanelImmersiveExit(immersiveShellRef.current, () => {
      // Clear leftover motion transforms â€” translated abspos hosts can inflate
  ...
  // beats nav-back / filter layers â€” a bubble-only keydown loses that race.
          ...
          window.dispatchEvent(new CustomEvent('bndz-native-alert', {
            detail: {
              ...
              message: result.latestVersion
                ? `BNDZ ${result.latestVersion} is available. Open Help â†’ About to download.`
  ...
  // Settings â†’ Check for language updates at startup
  useEffect(() => {
    ...
    void import('../lib/ipcBridge').then(({ IPC }) => {
      ...
      void IPC.checkForLanguageUpdates(manifestUrl).then(result => {
        ...
        const names = result.updates.map(u => `${u.id} ${u.installedVersion}â†’${u.latestVersion}`).join(', ');
        window.dispatchEvent(new CustomEvent('bndz-native-alert', {
          detail: {
  ...
  // Settings â†’ Maximum number of items cached
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (effectivePreviewOpen) {
      ...
      const apply = () => {
        try { panel.resize(panelPct(pct)); } catch { /* ignore */ }
        if (!previewDockedInWorkspace && outerGroupRef.current) {
          try {
            outerGroupRef.current.setLayout(computeVisibleOuterLayout(
              { ...(config.workspaceLayoutOuter || DEFAULT_OUTER_LAYOUT), preview: pct },
              uiRuntime.treePanel,
              true,
            ));
    ...
    previewPanelRef,
    previewDockedInWorkspace,
    config.workspaceLayoutOuter?.preview,
    config.workspaceLayoutMainRow?.preview,
    outerGroupRef,
    uiRuntime.treePanel,
    config.workspaceLayoutOuter,
  ]);
  ...
  /** Sticky header identity per pane â€” React updates only when the active group changes. */
  ...
  /** Last type-ahead hit â€” sync so same-key cycling works before React re-renders focusedItemId. */
  const typeAheadMatchIdRef = useRef<string | null>(null);
  const lastGlobalQueryRef = useRef('');
  /** True after the user clicks/focuses a file list â€” type-ahead stays armed even if WebView2 leaves chrome focused. */
  const listTypeAheadArmedRef = useRef(false);
  /** Latest type-ahead context â€” handler is stable so letter keys never hit a stale closure. */
  ...
  const openBottomPlugin = React.useCallback((pluginId: string, launch?: BottomPluginLaunchContext) => {
    // Mesh Drop is a dialog, not a bottom plugin â€” never open a phantom tab.
    ...
    // Absorbed / retired plugin IDs â†’ host plugin + launch tab hint.
    ...
    const launchMerged: BottomPluginLaunchContext | undefined = absorb?.tab
      ...
      : launch;
    // Never auto-install â€” only open plugins the user already has installed.
    // Exception: Open Terminal (remote-mesh + tab=terminal) is core FM on Native â€”
    ...
    if (!installedPluginIdSet.has(resolvedId) && !allowSystemTerminal) {
      ...
      setToastMessage(`â€œ${label}â€ isnâ€™t installed. Add it from Extension Hub.`, 'warning');
      return;
    }
  ...
  // Truncated first-paint when MORE fails â€” clear sticky Streaming and force one full refetch.
  useEffect(() => {
  ...
  /** Last HTML5 drag-over target â€” fallback when native drop coordinates miss. */
  ...
  /** Dest-relative paste flash: `normalizePanePath::nameLower` â†’ expiresAt ms (green tint until age filter catches up). */
  const recentPasteUntilRef = useRef(new Map<string, number>());
  /** Pending FS ops â€” keep tombstoned names filtered from cache until the queue job finishes. */
  ...
  const clearListDragGhost = React.useCallback((opts?: { immediate?: boolean }) => {
    fluidDragGenRef.current += 1;
    setListDragGhost(null);
    if (opts?.immediate) {
      // Hide ghosts only â€” never arm html.bndz-ole-drag-handoff here.
      // That class sets pointer-events:none on list/sidebar and was left stuck after
      // internal/spatial end-file-drag, making subsequent list/desktop drops feel broken.
      try {
        document.querySelectorAll('.bndz-fluid-drag-stack, .bndz-drag-ghost-root').forEach(el => {
  ...
  // Settings â†’ Resizing the window (dual pane): keep left/right pixel width fixed on window resize
  useEffect(() => {
  ...
  const cachePathContents = React.useCallback((path: string, data: any[], opts?: { retainLarger?: boolean }) => {
    ...
    setPathContentsCache(prev => {
      ...
      // Keep inbound/outbound optimistic rows until the server listing catches up â€”
      ...
      const mergeOptimistic = (server: any[]): any[] => {
        ...
        if (!existing?.length) return dedupeListingByName(stampRecent(server));
        ...
        if (!pending.length) {
          return dedupeListingByName(stampRecent(server));
        }
        const merged = config.addNewItemsAtTheEndOfTheList
          ? [...server, ...pending]
          : [...pending, ...server];
        return dedupeListingByName(stampRecent(merged));
      };
      // Progressive first-page RESULT must not shrink a fuller warm/streamed listing â€”
      ...
      if (
        ...
        // Full/soft refresh shrunk the folder â€” trust the server (deletes/moves), keep optimistic.
  ...
  // Progressive SharedBuffer append â€” merge remainder into first-paint list (never replace).
  useEffect(() => {
    const onAppend = (ev: Event) => {
      ...
      if (!detail?.items || !Array.isArray(detail.items) || !detail.items.length) {
        // Empty remainder (folder == first-paint size) â€” just clear streaming flag.
  ...
  // Mid-enumeration stream chunks â€” merge into live list while backend scans disk.
  useEffect(() => {
  ...
  /** Always-current navigate â€” sidebar modules must not close over a stale setCurrentPath. */
  const setCurrentPathRef = useRef<(path: string, paneId?: string, updateHistory?: boolean) => void>(() => {});
  /** Paths seeded by Files ShellViewModel (`BNDZ_DIR_LISTING`) â€” prefer over GET_DIR_CONTENTS. */
  const filesFedPathsRef = useRef(new Set<string>());
  /** Cancelable Files-feed wait per path â€” listing handler clears; never falls back to GET_DIR_CONTENTS. */
  ...
  useEffect(() => {
    ...
    const onOsQuickLookOpen = (ev: Event) => {
      const detail = (ev as CustomEvent).detail as {
        paths?: string[];
        items?: Array<{ path?: string; name?: string; isDirectory?: boolean }>;
      } | undefined;
      const rawItems = Array.isArray(detail?.items) ? detail!.items! : null;
      const paths = (detail?.paths ?? []).filter(Boolean);
      const source = rawItems?.length
        ? rawItems.filter((it) => typeof it?.path === 'string' && it.path)
      ...
      const items = source.map((it) => {
        ...
        const ext = String(it.entity.extension || '');
        return !isShellActivateExt(ext);
      });
      if (!items.length) return;
      setHomeQuickPreview({ items, index: 0 });
      setQuickPreviewStudio(false);
      setQuickPreviewOpen(true);
    };
    const onOsQuickLookClose = () => {
      setQuickPreviewOpen(false);
      setHomeQuickPreview(null);
      try {
        (window as any).chrome?.webview?.postMessage?.({ type: 'BNDZ_QUICK_LOOK_CLOSED' });
      } catch { /* ignore */ }
    };
    window.addEventListener('bndz-quick-look-open', onOsQuickLookOpen);
    window.addEventListener('bndz-quick-look-close', onOsQuickLookClose);
    return () => {
      ...
      window.removeEventListener('bndz-quick-look-open', onOsQuickLookOpen);
      window.removeEventListener('bndz-quick-look-close', onOsQuickLookClose);
    };
  }, []);
  ...
  const beginDirFetch = React.useCallback((rawPath: string, opts?: { force?: boolean }): Promise<void> | undefined => {
    ...
    // Cache hit: still re-warm shell glyphs / thumbs (Explorer feel â€” icons stay ready).
    ...
    // Craft islands (chrome/sidebar): host NativeList owns listing â€” navigate only.
    ...
    if (isHostNativeListFolder && isNativeShellHostBoot()) {
      ...
      return IPC.getDirContents(path).then(data => {
        ...
        const runPrefetch = () => prefetchListingVisuals(normalized, path, listingPrefetchFromConfig(configRef.current));
        if (typeof requestIdleCallback === 'function') {
          requestIdleCallback(() => runPrefetch(), { timeout: 1800 });
        } else {
    ...
    if (isVirtualCatalogPath(path)) {
      ...
      return IPC.getCatalogContents(path).then(data => {
        ...
        // Do not stamp sticky [] â€” leave undefined so the next navigate/open can retry.
    ...
    if (isBndzVirtualPath(path)) {
      if (isBndzPortalPath(path) || parseBndzPortalView(path)) {
        // Portal namespace retired â€” leftover /bndz/port/* paths remap in setCurrentPath.
      ...
      // fire create-all-at-once thumbnail storms â€” that OOM/kills WebView2 and leaves a blank shell.
    ...
    // HELLO_GATE_CHECK here â€” a check timeout used to replace the whole pane with
    ...
    const applyHelloGateBlock = (gatePath: string) => {
      ...
      // Empty cache so the pane leaves the infinite "Streamingâ€¦" skeleton and shows the gate UI.
    ...
    return IPC.getDirContents(path).then(data => {
      const normalized = normalizeDirEntries(data);
      // First-page resolve may be only 64 rows â€” keep fuller warm/streamed cache until APPEND.
      cachePathContents(path, normalized, { retainLarger: true });
  ...
  useEffect(() => {
    ...
    panes.forEach(pane => {
      ...
      if (!path) return;
      // Blend: Files ShellViewModel feeds BNDZ_DIR_LISTING â€” short fallback, not a 2.2s stall.
      if (isFilesHostBoot()) {
  ...
  const refetchPath = React.useCallback(async (rawPath: string) => {
    ...
    if (inFlight) {
      // Coalesce: another paste/refresh landed while listing â€” run once more after settle.
    ...
    try {
      await loadPromise;
      try {
        const { clearNativeContextMenuCache } = await import('../lib/nativeContextMenuCache');
        clearNativeContextMenuCache(toWindowsPath(path));
      } catch { /* optional */ }
    } finally {
  ...
  const commitRenameForEntity = React.useCallback(async (
    ...
    // Settings â†’ Directional formatting codes protection
    ...
    const renameLabel = `Rename: ${entity.name} â†’ ${displayTarget}`;
  ...
  const prefetchPathQuiet = React.useCallback(async (rawPath: string) => {
    const path = normalizePanePath(rawPath);
    if (!path) return;
    // Hover when listing already cached â€” still hydrate shells (plan #10).
    if (pathContentsCacheRef.current[path] !== undefined) {
      const cached = pathContentsCacheRef.current[path];
      if (Array.isArray(cached) && cached.length > 0) {
  ...
  useEffect(() => {
    ...
    return IPC.onIndexProgress(p => {
      if (p.jobComplete || (p.done && p.error)) {
        // Warm boot: all roots still fresh â€” do not flash a spinner/complete chip.
        ...
        // Per-root done â€” keep chip visible; more roots may follow in the same job.
  ...
  /** Last hover coords+state from the drag-hover RAF â€” used to skip duplicate hit-tests in onMove. */
  ...
  const oleEscalateOpIdRef = useRef<string | null>(null);
  useEffect(() => {
    const applyOleMoveRemove = (winPaths: string[], opts?: { invalidate?: boolean; opId?: string }) => {
      ...
      // Soft-refresh only after shell settle â€” early GET_DIR resurrects rows while Windows
    ...
    const resolveOlePaths = (raw?: string[] | null): string[] => {
      let winPaths = (Array.isArray(raw) ? raw : [])
        .map(p => String(p || '').replace(/\//g, '\\'))
        .filter(Boolean);
      if (!winPaths.length) {
        const stashed = peekOleDragSession();
        if (Array.isArray(stashed?.paths) && stashed!.paths.length) {
    ...
    // Wallpaper/desktop MOVE: strip list rows the moment OLE escalates (boundary handoff),
    // not only after OLE_DRAG_ENDED â€” otherwise the folder sits in the list for the whole drop.
    const onOleEscalated = (ev: Event) => {
      const detail = (ev as CustomEvent<{ paths?: string[] }>).detail;
      const winPaths = resolveOlePaths(detail?.paths);
      if (!winPaths.length) return;
      const oleOpId = oleEscalateOpIdRef.current || `ole-move-${Date.now()}`;
      oleEscalateOpIdRef.current = oleOpId;
    ...
    const onOleEnded = (ev: Event) => {
      ...
      const winPaths = resolveOlePaths(detail?.paths);
      const stashed = peekOleDragSession();
      const outboundFromBndz = !!stashed || winPaths.length > 0 || !!oleEscalateOpIdRef.current;
      const oleOpId = oleEscalateOpIdRef.current || `ole-move-${Date.now()}`;

      if (cancelled) {
        // Reinject rows stripped at escalate when the user cancelled the wallpaper drop.
        if (oleEscalateOpIdRef.current) {
          try {
        ...
        oleEscalateOpIdRef.current = null;
      ...
      // BNDZ-originated outbound drag â€” wallpaper/desktop often latches COPY/NONE while MOVE completes.
      if (winPaths.length > 0 && outboundFromBndz) {
        ...
        oleEscalateOpIdRef.current = oleOpId;
        // Always strip immediately for outbound drops â€” reinject only if disk still has the item.
        applyOleMoveRemove(winPaths, { invalidate: false, opId: oleOpId });
        if (oleVerifyTimer != null) window.clearTimeout(oleVerifyTimer);
        const verifyGone = (pass: number) => {
          // Prefer host sourcesGone when we already know â€” skip IPC storms that freeze WebView.
          if (sourcesGone && pass === 0) {
          ...
          void Promise.all(winPaths.map(p => IPC.checkPathExists(p).then(ex => !ex).catch(() => false)))
            .then(goneFlags => {
              ...
              if (allGone) {
                applyOleMoveRemove(winPaths, { invalidate: true, opId: oleOpId });
                // Keep tombstones longer so soft-refresh / retainLarger cannot resurrect.
                window.setTimeout(() => {
                  try {
                    for (const key of [...pendingFsOpsRef.current.keys()]) {
                      if (key === oleOpId || key.startsWith(`${oleOpId}:`)) clearFsTombstone(key);
              ...
              // ~5s of gentle polling â€” avoid IPC storms that freeze the WebView.
              if (pass < 12) {
                oleVerifyTimer = window.setTimeout(() => verifyGone(pass + 1), 400);
                return;
              }
              if (isMove || sourcesGone) {
                applyOleMoveRemove(winPaths, { invalidate: true, opId: oleOpId });
                window.setTimeout(() => { oleEscalateOpIdRef.current = null; }, 5000);
                return;
              }
              // Still on disk after polls + COPY latch â€” true copy; reinject rows.
              try {
                for (const key of [...pendingFsOpsRef.current.keys()]) {
                  if (key === oleOpId || key.startsWith(`${oleOpId}:`)) reinjectFsTombstone(key);
                }
        ...
        oleVerifyTimer = window.setTimeout(() => verifyGone(0), sourcesGone ? 40 : 180);
      } else {
        oleEscalateOpIdRef.current = null;
      }
      nativeOleDragRef.current = false;
    ...
    window.addEventListener('bndz-ole-drag-escalated', onOleEscalated);
    window.addEventListener('bndz-ole-drag-ended', onOleEnded);
    return () => {
      window.removeEventListener('bndz-ole-drag-escalated', onOleEscalated);
      window.removeEventListener('bndz-ole-drag-ended', onOleEnded);
      if (oleVerifyTimer != null) window.clearTimeout(oleVerifyTimer);
    };
  }, [clearListDragGhost, clearFsTombstone, reinjectFsTombstone]);
  const suppressRowClickRef = useRef(false);
  /** Gesture double-tap already activated â€” ignore trailing native dblclick (~700ms). */
  const suppressNativeDblUntilRef = useRef(0);
  /** WebView2 often synthesizes list-body clicks after a row press â€” block deselect briefly. */
  ...
  const startColumnResize = (colId: ListColumnId, startX: number, headerEl: HTMLElement) => {
    ...
    const finish = () => {
      ...
      // Read latest config via ref â€” mid-drag updates must not clobber sibling widths.
      const cfg = configRef.current;
      const folderWidths = {
  ...
  const runUndoRedo = React.useCallback(async (redo = false) => {
    ...
    // Always ask the host â€” FE canUndo can lag behind deletes (ACTION_LOG_CHANGED race).
    ...
    pushToast({ id: toastId, kind: 'progress', title: redo ? 'Redoingâ€¦' : 'Undoingâ€¦', message: 'Please wait', sticky: true });
    try {
      const r = redo ? await executeRedoWithTimeout() : await executeUndoWithTimeout();
      dismissToast(toastId);
      refreshUndoRedoState();
      if (isQueuedIpcResult(r)) {
        pushToast({ kind: 'info', title: redo ? 'Redo queued' : 'Undo queued', message: 'Running in the transfer panelâ€¦' });
        return;
      }
      if (r.ok) {
     ...
     if (config.enableGlobalSearchPrefix && filterText.trimStart().startsWith('> ')) {
         ...
         if (query.length > 0) {
             ...
             const timer = setTimeout(() => {
                 ...
                 // Settings â†’ Use space character for Boolean AND
  ...
  useEffect(() => {
    ...
    // Settings â†’ Thumbnails â†’ Include search results (disk cache / full visual warm)
  ...
  useEffect(() => {
    ...
    // Wait until config has had a chance to load tabset id (undefined â†’ settled).
    ...
    if (config.restoreLastTabsetOnStartup !== false && config.lastActiveTabsetId && !restoredTabsetRef.current) {
      // Tabset restore effect may still run â€” apply after a microtask if it didn't.
      const t = window.setTimeout(() => {
  ...
  // Permanent Continuum Home tab â€” locked Home in every pane when enabled.
  useEffect(() => {
  ...
  // Match OS light/dark â€” startup one-shot and/or continuous follow
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      // Quick Look overlay (Space) â€” skip while open/IME; audio keeps playing via shared session
      ...
      if (!isInput && matchesShortcut(e, keyboardMap.refresh)) {
         ...
         // Status-bar style feedback only â€” avoid toast/notification spam on every F5.
      }
      ...
      if (!isInput && (matchesShortcut(e, keyboardMap.copy) || isCutShortcut)) {
          ...
          if (tab && tab.selectedItems.length > 0) {
                 ...
                 if (selectedEntities.length > 0) {
                     ...
                     // Status-bar clipboard chip is the primary cue (Explorer-quiet) â€” pulse it.
      ...
      // Smart Tools (Ctrl+Shift+A) â€” opens Assistant tab
      ...
      // View mode chords â€” Ctrl+Shift+1/2/3/4 (details / list / grid / columns)
      ...
      // Inline rename â€” list item or pinned Rapid access favorite label
      ...
      // Alt+Enter â€” properties (Explorer standard)
  ...
  /** True when openMenuId was switched by hover-chaining â€” next click must not toggle-close. */
  ...
  // Optionally merge Windows shell "New" cascade into the File â†’ New menubar when available.
  useEffect(() => {
  ...
  const handleContextMenuRequest = async (
    ...
    entityExtension?: string | null,
      ...
      // Settings â†’ Use custom context menu (false â†’ host shell menu only)
      ...
      // Shift+right-click â†’ real host shell menu (multi-select aware).
      ...
      if (mergeShellVerbs && !isVirtualLocation && shellPaths.length > 0) {
        try {
          ...
          // Exact path OR extension/shape â€” paint immediately when warm.
          const cachedNative = lookupNativeContextMenu(shellPaths) as any[] | null;
          if (cachedNative?.length) {
            initialNative = cachedNative;
            // Background refresh â€” signature gate avoids height thrash when unchanged.
            void (async () => {
              try {
                const { IPC } = await import('../lib/ipcBridge');
            ...
            // No skeleton â€” if still cold after budget, paint BNDZ verbs and patch in silently.
  ...
  const sidebarInputRegionsNudgedRef = React.useRef(false);
  ...
  // NOTE: must NOT be useCallback with thin deps â€” that froze the first render's
  ...
  const sidebarNavigateFromPointer = (path: string, e?: React.PointerEvent | React.MouseEvent | { button?: number }) => {
    ...
    if (!path) return;
    // Nudge WinUI caption Passthrough on first sidebar interaction after boot â€”
    // regions often settle late and leave LMB dead for seconds.
    if (!sidebarInputRegionsNudgedRef.current) {
      sidebarInputRegionsNudgedRef.current = true;
      try { IPC.windowChrome('refreshInputRegions'); } catch { /* ignore */ }
    }
    // Only debounce true double-fires (pointerdown + click), not intentional rapid nav.
    if (Date.now() - lastSidebarNavAtRef.current < 40) return;
  ...
  const handleDeleteRequest = (items: any[], path: string, isFromTree: boolean = false, options?: { permanent?: boolean }) => {
    ...
    if (isPortableDeviceReadOnly(path, config.treatPortableDevicesAsReadOnly === true)) {
      pushToast({
        ...
        message: 'Turn off â€œTreat portable devices as read-onlyâ€ in Settings to allow deletes on phones/MTP.',
      });
      return;
    }
    const rt = buildSettingsRuntime(config);
    const listIx = getListIxBehavior(config);
    // Recycle when deleteToRecycleBin is on and bypassRecycleBin is off (Shift+Delete â†’ permanent).
    ...
    const executeDelete = () => {
      ...
      // Progress lives only in FileTransferQueuePanel â€” no center sticky toasts.
      IPC.executeFsOperation(opId, 'delete', winPaths, '', bypassRecycle, label, 'high');
      // Skip aggressive RAM refetch â€” tombstones keep rows hidden until the job completes.
      })();
    };
    ...
    const confirmMsg = items.length === 1
      ...
      : `${options?.permanent ? 'Permanently delete' : 'Delete'} ${items.length} items?${bypassRecycle ? '\n\n(Bypassing Recycle Bin)' : '\n\nItems will be moved to the Recycle Bin.'}\n\nâ€¢ ${names}${items.length > 5 ? '\nâ€¢ ...' : ''}`;

    // Permanent deletes (Shift+Delete / bypass) always require an explicit confirmation â€” never skippable.
  ...
  const createNewItemInActivePane = async (name: string, kind: 'dir' | 'file') => {
    ...
    // Optimistic row so select/rename work even if refetch lags the FS watcher.
    {
      ...
      setPathContentsCache(prev => {
        ...
        const nextList = dedupeListingByName(config.addNewItemsAtTheEndOfTheList ? [...existing, stub] : [stub, ...existing]);
  ...
  /** Mandatory confirmation for irreversible Recycle Bin purge â€” never skippable. */
          ...
          action: () => {
            void (async () => {
              ...
              if (isQueuedIpcResult(result)) {
                setToastMessage('Delete queued â€” see transfer panel.');
                return;
              }
              if (result.purged > 0) {
                setToastMessage(
      ...
      actions: [
        {
          ...
          style: 'destructive',
          action: () => {
            setToastMessage('Emptying Recycle Binâ€¦');
            import('../lib/ipcBridge').then(({ IPC }) => {
              IPC.emptyRecycleBin().then(result => {
                if (isQueuedIpcResult(result)) {
                  setToastMessage('Empty Recycle Bin queued â€” see transfer panel.');
                  return;
                }
                if (result?.success) {
                  ...
                  setPathContentsCache(prev => ({
                    ...prev,
                    [RECYCLE_BIN_PATH]: [],
                    [normalizePanePath(RECYCLE_BIN_PATH)]: [],
                  }));
                  pathContentsCacheRef.current[RECYCLE_BIN_PATH] = [];
                  pathContentsCacheRef.current[normalizePanePath(RECYCLE_BIN_PATH)] = [];
                  filesFedPathsRef.current.delete(RECYCLE_BIN_PATH);
                  filesFedPathsRef.current.delete(normalizePanePath(RECYCLE_BIN_PATH));
                  void refetchPath(RECYCLE_BIN_PATH);
                  refreshWorkspace();
                } else {
                  setToastMessage(result?.error ?? 'Could not empty Recycle Bin.');
                  void refetchPath(RECYCLE_BIN_PATH);
                }
              }).catch(() => {
  ...
  // Clipboard cut-paste and other surfaces register optimistic tombstones via this event.
  useEffect(() => {
    const onOptimistic = (ev: Event) => {
      ...
      if (detail.kind === 'move' || detail.kind === 'delete') {
        xferMetaRef.current.set(detail.opId, {
          ...
          label: detail.label || names[0] || 'items',
        });
        setPathContentsCache(prev => {
          let next = prev;
          for (const parent of sourceParents) {
    ...
    window.addEventListener('bndz-optimistic-fs-op', onOptimistic as EventListener);
  ...
  useEffect(() => {
    ...
    const unsub = IPC.onFileTransferQueueChanged(state => {
      ...
      // use a shorter (or zero) refresh delay â€” tombstones already removed the rows
      ...
      for (const job of state.jobs) {
        ...
        // Throttled mid-run dest refresh â€” keeps optimistic rows stable on small ops.
        ...
        if (job.status === 'completed' || job.status === 'failed' || job.status === 'cancelled') {
          if (!completed.has(job.operationId)) {
            ...
            if ((isDelete || isMove || isRename) && (job.status === 'failed' || job.status === 'cancelled')) {
              ...
              // Instant reinject â€” don't wait for disk refresh (blank gap after optimistic hide).
              reinjectFsTombstone(job.operationId);
              // Clear backend filter BEFORE refresh â€” fire-and-forget clear raced refetch and
              // left rows missing until F5 (listing still tombstone-filtered on host).
              failedTombstoneClears.push(job.operationId);
            } else if ((isDelete || isMove) && job.status === 'completed') {
              // Keep tombstones through disk refresh â€” prevents flicker-back when listing lags AV/recycle.
            ...
            if (job.status === 'failed') {
              ...
              // Rename invalidName also gets a dedicated sheet (Fix name) â€” E4.14.
              const showOpsSheet =
...
Restart BNDZ as administrator to retry?` },
                ...
                if (classified.kind === 'intoSelf') {
                  ...
                  const fixInvalidName = async () => {
                    ...
                    // Copy/move: retry after user picks a legal leaf â€” host still owns dest naming;
                    // surface toast so they can rename the source first if needed.
                    pushToast({
                      kind: 'info',
                      title: 'Rename the source',
                      message: `Use a valid name like â€œ${next.trim()}â€, then Retry the transfer.`,
                    });
                    retryThisTransfer();
                  };
                  actions = [
...
${classified.detail}`,
              ...
              // Floating toast / transfer panel already surface success â€” skip center success spam
      ...
      if (shouldRefresh) {
        ...
        // Instant confirmation refresh for ordinary FS ops â€” optimism already updated the list.
  ...
  useEffect(() => {
    ...
    // pointerdown captures earlier than click â€” WebView2 sometimes eats the click after drag.
  ...
  // Settings â†’ Resolve cache path from current folder
  useEffect(() => {
  ...
  // Keep keyboard focus aligned with list selection (marquee / programmatic selects).
  useEffect(() => {
    const sel = activeTab.selectedItems || [];
    if (!sel.length) return;
    if (focusedItemId && sel.includes(focusedItemId)) return;
    setFocusedItemId(sel[0]);
  }, [activeTab.selectedItems.join('|'), activePaneId]);
  ...
  const workspaceToolActive = useMemo(() => {
    ...
    const tab = pane.tabs[pane.activeTabIndex];
    const path = normalizePanePath(tab?.path || '');
    return isBndzImmersiveWorkspacePath(path);
  }, [panes, activePaneId]);
  ...
  useEffect(() => {
    ...
    // Home / Spatial / Automation must force the dock shut â€” CSS is belt-and-suspenders;
  ...
  useEffect(() => {
    ...
    if (!path || isBndzVirtualPath(path) || isRecycleBinPath(path) || !isValidShellTarget(path)) return;
  ...
  const addressSuggestions = useMemo(() => {
    ...
    // Settings â†’ Auto-Complete Path Names â†’ Address Bar
    ...
    // Controls â†’ Auto-complete recently used items gates history visits in the dropdown.
  ...
  /** Live density while scrubbing â€” avoid config/save/runtime churn every pointer move. */
  ...
  // Settings â†’ Persist visual filters across folders (kind/tag list filters)
  ...
  // Cancel in-flight scan when leaving a path â€” keep completed signatures so revisits skip IPC.
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    // Never contend with cold list/icon pipe right after navigate (native + files-host).
    const deferMs = 8000;
    let cancelled = false;
  ...
  useEffect(() => {
    ...
    if (!currentDirCount) return;
    // Same scheduler as above when both flags are on â€” skip duplicate kick.
  ...
  useEffect(() => {
    ...
    import('../lib/ipcBridge').then(({ IPC }) => {
      unsubscribe = IPC.onFsEvents((events) => {
        ...
        const allowDuringOps = rt.operations.refreshDuringOps || !!config.refreshDuringFileOperations;
        ...
        if (shouldRefresh) {
          ...
          for (const ev of events) {
            // Mid-transfer: still apply Created/Deleted/Renamed so paste/rename appear live.
            // Skip Changed-only churn unless the user opted into refresh-during-ops.
            if (transfersBusy && !allowDuringOps) {
              const t = String(ev.type || '');
              if (t !== 'Created' && t !== 'Deleted' && t !== 'Renamed') continue;
            }
            ...
            // RAM staging: host watches the real mount â€” map back to open /bndz/ram tabs.
          ...
          byDir.forEach((dirEvents, panePath) => {
            ...
            if (next && next !== cached) {
              setPathContentsCache(prev => setPathCacheEntry(prev, panePath, dedupeListingByName(next)));
            }
            const meaningful = dirEvents.some(e => {
              const t = String(e.type || '');
              return t === 'Created' || t === 'Deleted' || t === 'Renamed';
            });
            // Always soft-refresh the open listing on Created/Deleted/Renamed â€”
            // patch alone often lacks host metadata and left paste/rename invisible.
            if (meaningful) {
              scheduleSoftRefresh(panePath);
            } else if (needsSoftRefresh || !cached) {
              scheduleSoftRefresh(panePath);
            }
          });
        }
        // Settings â†’ Auto-refresh tags: re-fetch listing so sidecar tags re-enrich
  ...
  useEffect(() => {
    ...
    import('../lib/ipcBridge').then(({ IPC }) => {
      unsubProp = IPC.onProgress((progressDetails) => {
        ...
        // happened to carry percentage=100/0 â€” check explicitly before any pct-based branch.
        ...
        // Live percent belongs in FileTransferQueuePanel only.
  ...
  /** During pointer file drag â€” drives list + tab chrome to hovered tab before drop. */
  ...
  const libraryFolderItems = useMemo(() => {
    ...
    const add = (name: string, rawPath: string) => {
      ...
      items.push({
        ...
        iconPath: KNOWN_FOLDER_SHELL[name]
          ...
          : p,
        // Navigate-only under Libraries â€” expanding here surfaced a nested
    ...
    for (const name of order) {
      ...
      // C:\Users\â€¦\Desktop instead of shell:Desktop.
  ...
  /** List-pane entities for Libraries â€” same known folders as the tree (shell enum often returns blank names). */
  ...
  /** Drives shown in This PC / tree / letter bar â€” exclude dedicated cloud volume letters only,
  ...
  const navigationDrives = useMemo(() => {
    ...
    return drives.filter(d => {
      ...
      // letter is "C:" â€” check both with and without colon
  ...
  const refreshNetworkLocations = React.useCallback(() => {
    const netBeh = getNetworkBehavior(config);
    if (netBeh.noNetworkBrowsingAtStartup) return;
    void import('../lib/ipcBridge').then(({ IPC }) => {
      IPC.getNetworkLocations({
        assumeMappedReady: !!netBeh.assumeThatMappedNetworkDrivesAreAvailable
          || !!config.assumeThatMappedNetworkDrivesAreAvailable
          ...
          || !!netBeh.assumeThatServersAreAvailable,
        cacheServers: !!netBeh.cacheNetworkServers || !!config.cacheNetworkServers,
      }).then(n => setNetworkNodes(Array.isArray(n) ? n : []));
    });
  }, [config]);
  ...
  // Path-only deps + refs â€” avoid updateConfig/openBottomPlugin identity storms.
    ...
    const timer = window.setTimeout(() => {
      void (async () => {
        try {
          ...
          // Do not auto-open/install plugins on navigate â€” that chained ensurePluginInstalled
          // â†’ updateConfig â†’ shell apply and contributed to host freezes.
        } catch {
          /* contract optional */
        }
      })();
  ...
  const treeData = useMemo(() => {
    const raw: NavTreeBuildNode[] = [
      {
        ...
        label: 'Home',
        path: BNDZ_HOME,
        // Native Profile shell glyph; Icons8 home is launch house â€” avoid as fallback.
      ...
      },
      {
        ...
        icon: 'sparkles_ui',
        iconColor: '#0078d4',
        // Virtual /bndz paths are not shell namespace items â€” shell fetch â†’ white file placeholders.
        useShellIcon: false,
      ...
      {
        treeKey: 'workspace-tools',
        draggable: true,
        label: 'Workspace Tools',
        // No path â€” parent must NOT share /bndz/canvas with Spatial Canvas child
        // (duplicate React row ids were painting a second "Workspace Tools" ghost).
        icon: 'layers_ui',
        iconColor: '#34d399',
        useShellIcon: false,
        ...
        childrenItems: [
          ...
          },
          {
            ...
            iconColor: '#34d399',
            useShellIcon: false as const,
            onClick: () => openWorkspaceToolTab(BNDZ_AUTOMATION),
          },
        ],
      },
      ...
      },
      {
        ...
        onToggle: () => {
          const next = !linuxExpanded;
          setLinuxExpanded(next);
          if (next && wslDistroNodes.length === 0) refreshNetworkLocations();
        },
      ...
      ...(config.meshShowInNavTree !== false && meshHosts.some(h => h.showInNavTree !== false) ? [{
        ...
        childrenItems: meshHosts
          ...
          .map(h => {
            ...
            return {
              label: isEphemeral ? `${h.alias} Â· Ephemeral` : h.alias,
    ...
    // Intentionally omit currentPath â€” selection highlight is handled per-row via
    // panePathsEqual; rebuilding the whole tree on every navigate freezes Inbound/Problems.
    drives, navigationDrives, thisPcExpanded, libraryFolderItems,
  ...
  useEffect(() => {
    ...
    const walk = (nodes: typeof treeData) => {
      for (const n of nodes) {
        // Virtual bndz views skip shell prefetch unless they declare iconPath (e.g. Continuum Home â†’ shell:Profile).
  ...
  const toggleDualPane = () => {
    ...
    if (isDualPane) {
       if (tabLimits.alwaysKeep1stPaneVisible && panes.length >= 2) {
         // Keep pane 1 (first) visible â€” collapse by removing the inactive/other pane only.
  ...
  const toggleViewLock = (paneId: string) => {
    setPanes(prev => prev.map(p => {
      ...
      if (tab.viewLocked) {
        ...
        setToastMessage('View locked â€” sort, filter, and columns frozen.');
      }
      return { ...p, tabs };
    }));
  };
  ...
  const runAddressQuickScriptHandler = (raw: string, paneId: string) => {
    ...
    return runAddressQuickScript(raw, {
      ...
      // Settings â†’ Remember permanent variables
   ...
   *   %AppData%, %LOCALAPPDATA%, %TEMP%, %USERPROFILE% â€” any %VAR%
   *   C:\path, D:\ etc.       â€” drive letters
   *   shell:Downloads etc.    â€” shell known folders
   *   \\server\share          â€” UNC paths
   *   ::scripts               â€” address-bar quick scripts
   *
   * Command mode ('>' prefix):
   *   >refresh / >reload      â€” reload active folder
   *   >dual / >split          â€” toggle dual pane
   *   >preview / >inspector   â€” toggle preview panel
   *   >settings / >config     â€” open settings dialog
   *   >rename                 â€” batch rename plugin
   *   >find [query]           â€” fast search plugin (or new finding tab if query given)
   *   >search [query]         â€” alias for >find
   *   >go / >cd [path]        â€” navigate
   *   >metadata               â€” metadata inspector plugin
   *   >tabset                 â€” save tabset
   *   >palette / >commands    â€” open command palette
   *   >plugins / >store / >hub â€” Extension Hub
   */
  const tryOmnibarSubmit = async (raw: string): Promise<boolean> => {
    ...
    if (trimmed.startsWith('>')) {
      ...
      return true; // consumed â€” error toast shown
    }
  ...
  const openFolderContentsPeek = async (
    ...
    // Settings â†’ Folder contents preview (+ In list / In tree + mouse-up buttons)
  ...
  const executeInternalDrop = (
    ...
    void (async () => {
      ...
      if (isMeshDropSend) {
        // Mesh Drop opens the pairing dialog â€” no fake progress toast.
        dismissToast(`xfer-${opId}`);
      }
      ...
      if (route.kind !== 'local') {
        ...
        if (!result.ok) {
          ...
          pushToast({ kind: 'info', title: 'Mesh Drop', message: 'Pairing dialog opened â€” share the Mesh Code with your peer.' });
        }
        return;
      }
      ...
      // Mesh paths must never fall through to Windows FS ops (toWindowsPath mangles /mesh/â€¦).
      if (isMeshPath(destCanon) || canonSources.some(isMeshPath)) {
        ...
        pushToast({
          kind: 'warning',
          title: 'Mesh transfer',
          message: isMeshPath(destCanon)
            ? 'Open a connected mesh host folder before dropping files.'
            : 'Mesh download needs a local folder destination.',
        });
        return;
      }

      // Budget governor â€” soft warn / hard block before local copy/move lands.
      try {
        if (IPC.isNative) {
          let incomingBytes = 0;
        ...
        /* governor / policy optional â€” never block drops on IPC failure */
      }

      // Drop onto RAM zone root â†’ stage API (preserves folder trees into the mount).
      ...
      if (destPane && isFsDropTargetPath(destPane)) {
        ...
        // Soft invalidate after paint â€” paste listener already owns optimism.
        window.setTimeout(() => {
          try {
      ...
      // Tombstones keep source rows hidden â€” avoid 200ms RAM refresh fighting optimistic UI.
  ...
  useEffect(() => {
    const onRetryLast = (ev: Event) => {
      ...
      // Prefer host failedPaths[] (partial batch) â€” only resubmit what actually failed.
  ...
  const openMenubarMenu = (menuId: string) => (e: React.MouseEvent) => {
    if (e.button !== 0) return;
    e.stopPropagation();
    if (menubarOpenedByHoverRef.current && openMenuId === menuId) {
      menubarOpenedByHoverRef.current = false;
      return;
    }
    menubarOpenedByHoverRef.current = false;
  ...
  /** Prefer live list cache over the empty virtual FS tree â€” toolbar/menubar/hotkeys must use this. */
  ...
  const navigateToAppDataFolder = async (kind: 'ini' | 'appdata' | 'app') => {
    ...
    if (kind === 'ini') {
      ...
      setToastMessage('Opening configuration file locationâ€¦');
      return;
    }
  ...
  const duplicateSelectedItems = async () => {
    ...
    // Same-folder copy via Copy To path â€” dest optimism + select + green tint (host collision rename).
  ...
  const pasteIntoNewSubfolder = async () => {
    ...
    const folderName = r.finalName || name;
    const destPane = isBndzRamPath(currentTab.path)
      ? `${normalizePanePath(currentTab.path).replace(/\/$/, '')}/${folderName}`
      : joinPanePathForFs(currentTab.path || '/', folderName);
  ...
  const pasteHereAs = async () => {
    ...
    if (clipboard.items.length !== 1) {
      setToastMessage('Paste Here Asâ€¦ works with one clipboard item.');
      return;
    }
    const src = toWindowsPath(clipboard.items[0]);
    const base = src.split(/[/\\]/).pop() || 'item';
    const raw = await requestNativePrompt({
      title: 'Paste Here Asâ€¦',
    ...
    const pasteAsRes = await IPC.executeFsOperation(`paste-as-${Date.now()}`, op, src, destFile, false, asName, 'high');
    if (clipboard.action === 'cut') clearClipboard();
    if (isQueuedIpcResult(pasteAsRes)) {
      setToastMessage(`Paste as "${asName}" queued â€” see transfer panel.`);
      return;
    }
    setToastMessage(pasteAsRes?.ok === false ? (pasteAsRes.error || 'Paste failed.') : `Pasted as "${asName}".`);
    if (pasteAsRes?.ok !== false) refreshWorkspace();
  };
  ...
  const pasteFolderStructureOnly = async () => {
    ...
    for (const src of clipboard.items) {
      ...
      // Prefer creating a folder named after each clipboard item (files â†’ folder of parent name skipped; use basename without ext for files)
      const folderName = base.includes('.') && !base.endsWith('\\') ? base.replace(/\.[^.]+$/, '') || base : base;
      const target = `${dest}\\${folderName}`;
      const res = await IPC.executeFsOperation(`paste-struct-${Date.now()}-${created}`, 'create-dir', target, '', false, folderName);
      if (isQueuedIpcResult(res)) queued += 1;
      else if (res?.ok !== false) created += 1;
    }
    if (queued) setToastMessage(`Folder create queued (${queued}) â€” see transfer panel.`);
    else setToastMessage(created ? `Created ${created} folder(s) from clipboard structure.` : 'Nothing to create.');
    if (!queued) refreshWorkspace();
  };
  ...
  const pasteExtractedFromClipboard = async () => {
    ...
    setToastMessage(`Extracting ${archives.length} archive(s)â€¦`);
    refreshWorkspace();
  };
  ...
  const pasteZippedFromClipboard = async () => {
    ...
    setToastMessage(isQueuedIpcResult(res) ? 'Zip queued â€” see transfer panel.' : (res.ok ? 'Clipboard items zipped.' : (res.error || 'Zip failed.')));
    refreshWorkspace();
  };
  ...
  const pasteTextAsItems = async () => {
    try {
      ...
      for (const line of lines) {
        ...
        const res = await IPC.executeFsOperation(`paste-text-item-${Date.now()}-${safe}`, 'create-file', path, '', false, safe);
        if (isQueuedIpcResult(res)) queued += 1;
        else if (res?.ok !== false) created += 1;
      }
      if (queued) setToastMessage(`Create queued (${queued}) â€” see transfer panel.`);
      else setToastMessage(`Created ${created} item(s) from clipboard text.`);
      if (!queued) refreshWorkspace();
    } catch {
  ...
  const applyTagToSelection = async (tag: { id?: string; name?: string; label?: string; color?: string }) => {
    ...
    // Fetch every sidecar once, in parallel â€” previously this fetched each path's
    ...
    // Settings â†’ Confirm copying tags (confirm before applying)
  ...
  /** Spatial / Automation â€” focus existing tool tab or open a dedicated closable tab. */
  const openWorkspaceToolTab = (path: string, paneId: string = activePaneId) => {
    const norm = normalizePanePath(path);
    const pane = panes.find(p => p.id === paneId);
    if (!pane) {
      addTab(paneId, norm);
    ...
    const existing = pane.tabs.findIndex(t => normalizePanePath(t.path) === norm);
    if (existing >= 0) {
      setPanes(prev => prev.map(p => (p.id === paneId ? { ...p, activeTabIndex: existing } : p)));
      return;
    }
    addTab(paneId, norm);
  };
  ...
  const closeTabAt = async (paneId: string, tabIndex: number, e?: React.MouseEvent) => {
    ...
    if (!pane || !tab) return;
    // Last tab: workspace tools can still close (reset to Home). Plain FS last tab stays.
    if (pane.tabs.length <= 1) {
      if (isBndzCanvasPath(tab.path) || isBndzAutomationPath(tab.path) || isBndzWorkspacePath(tab.path)) {
        const home = BNDZ_HOME;
        setPanes(prev => prev.map(p => {
          if (p.id !== paneId) return p;
          const only = p.tabs[0];
          if (!only) return p;
          return {
            ...p,
            tabs: [{ ...only, path: home, history: [home], historyIndex: 0, selectedItems: [], viewMode: undefined }],
            activeTabIndex: 0,
          };
        }));
  ...
  const setTabColor = (paneId: string, tabIndex: number, color: string) => {
    ...
    // and a void return caused React to open again after dismiss â€” double menus / flaky clicks.
  ...
  const applyFileDragHoverAtPoint = React.useCallback((clientX: number, clientY: number) => {
    ...
    setPointerDragHover(clientX, clientY, overList, {
      navTreePath: hover.navTreePath,
      breadcrumbPath: hover.breadcrumbPath,
      listFolderId: dragTargetIdRef.current,
    });
  ...
  useEffect(() => {
    // Coalesce hover updates â€” accumulate coords into a pending ref, flush once per animation
  ...
  const setCurrentPath = (path: string, paneId: string = activePaneId, updateHistory: boolean = true) => {
    ...
    // Settings â†’ Auto-create any missing folders
    if (
    ...
    setPanes(prev => prev.map(p => {
      if (p.id === paneId) {
        ...
        newTabs[p.activeTabIndex] = {
          ...
          historyIndex: newHistoryIndex,
          selectedItems: [],
          // View bar is sole authority â€” smart views filter content, never force layout.
          viewMode: tab.viewMode,
  ...
  // FilesMerge tabs â†’ BNDZ cwd (avoid echo loops via normalized compare).
  useEffect(() => {
  ...
  // Files engines â†’ BNDZ list (blend): seed cache and skip GET_DIR_CONTENTS for that path.
  useEffect(() => {
    if (!isFilesHostBoot()) return;
  ...
  useEffect(() => {
    ...
    const onOpenInBndz = (e: Event) => {
      ...
      const win = toWindowsPath(path);
      import('../lib/ipcBridge').then(({ IPC }) => {
        IPC.recordPathOpen(win);
        IPC.executeContextMenuVerb(win, 'open');
      });
    };
  ...
  useEffect(() => {
    const onMeshEdit = (e: Event) => {
      ...
      setToastMessage('Remote edit armed â€” dispatch bndz-mesh-write-back after saving the hydrated file.', 'success');
    };
  ...
  useEffect(() => {
    const onAddShared = (e: Event) => {
      ...
      openBottomPlugin('remote-mesh');
      setToastMessage(next.length > prev.length ? 'Added to Shared Libraries (Remote â†’ Buckets & Shares).' : 'Already in Shared Libraries.');
    };
  ...
  // Native OLE drag-hover: warm drop targets (list / tree / breadcrumb / tabs) â€” no HTML5.
  // C4.2: arm FluidDragStack for inbound desktopâ†’list when host sends path sample.
  useEffect(() => {
    ...
    const onExternalDragHover = (e: Event) => {
      ...
      // Outbound BNDZ OLE / in-app file-drag owns FluidDrag â€” never double-arm inbound.
  ...
  // Host-side inbound commit fallback â€” paint dest rows + select + tint (same as paste/drop).
  useEffect(() => {
  ...
  // Host: AllowExternalDrop=true (except BNDZ OLE) â€” Path A file: nav + Path B WPF PreviewDrop + forceCommit.
  useEffect(() => {
    const onExternalDrop = (e: Event) => {
      ...
      // Spatial / Automation own inbound drops â€” never let preview loupe swallow list drops.
    ...
    const onExternalDropFailed = (e: Event) => {
      ...
      setToastMessage(
        typeof reason === 'string'
          ...
          : 'Drop failed â€” no file paths extracted.',
      );
    };
  ...
  useEffect(() => {
    ...
    IPC.postOleDndDebug({
      kind: 'drag-policy',
      disallowList: !!config.disallowLeftDraggingFromFileList,
      thumbDrag: !!config.allowDraggingItemsByTheThumbnail,
      bgWindow: !!config.allowDraggingFromABackgroundWindow,
      fluidDrag: config.fluidDragStacks !== false,
      bgProcessing: config.enableBackgroundProcessing !== false,
      queueOps: config.queueFileOperations !== false,
    });
  }, [
    config.disallowLeftDraggingFromFileList,
    config.allowDraggingItemsByTheThumbnail,
    config.allowDraggingFromABackgroundWindow,
    config.fluidDragStacks,
    config.enableBackgroundProcessing,
    config.queueFileOperations,
  ]);
  ...
  useEffect(() => {
    ...
    void import('../lib/ipcBridge').then(({ IPC }) => {
      ...
      // Dense FE nudges â€” WinUIEx / NonClientRegionSupport settle late; list select
  ...
  useEffect(() => {
    ...
    import('../lib/ipcBridge').then(({ IPC }) => {
      unsub = IPC.onOpenPath(path => {
        if (!path) return;
        const norm = normalizePanePath(path);
        // Settings â†’ Open command line start path in a new tab
        if (config.openCommandLineStartPathInNewTab) {
          addTab(activePaneId, norm);
          return;
        }
        // Settings â†’ Open new instance always (multi-instance)
  ...
  /** Global Escape â€” dismiss stuck overlays in priority order */
  useEffect(() => {
    const layers = [
      registerEscapeLayer({
        id: 'file-drag-chrome',
        priority: 970,
        isActive: () => isOleDragHandoffActive()
          || !!getFileDragSession()
          || pointerFileDragActiveRef.current
    ...
    bottomImmersive, exitBottomImmersive, clearListDragGhost, clearFileDragChrome,
  ]);
  ...
  marqueeOpsRef.current.setFocusedItemId = setFocusedItemId;
  marqueeOpsRef.current.setMarqueePreviewIds = setMarqueePreviewIds;
  ...
  const executeFolderSync = async (mode: 'mirror' | 'updateTarget') => {
      ...
      try {
          ...
          if (isQueuedIpcResult(res)) {
              pushToast({ kind: 'info', title: 'Sync queued', message: 'Running in the transfer panelâ€¦' });
              return;
          }
          if (res.ok) {
              pushToast({
  ...
  const refreshActiveList = () => {
    ...
    // Single force-refetch for the active tab â€” do not also fan out invalidatePath
  ...
  const getSortedContentsForActivePane = React.useCallback(() => {
    ...
    if (isGlobal) {
      ...
      // Settings â†’ Find â†’ Let folders pass all filters
  ...
  // Keyboard state â€” type-ahead find in file list (Explorer-style)
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      // Modals / settings own keyboard â€” never type-ahead into the list behind them.
      ...
      // Settings â†’ Skip single spaces: Space alone does not advance type-ahead.
  ...
  // Keyboard state (navigation, shortcuts â€” separate from type-ahead capture handler)
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      const isInput = e.target instanceof HTMLInputElement || e.target instanceof HTMLTextAreaElement;
      ...
      if (e.key === 'Tab') {
        ...
        // Find â†’ Enable navigation keys gates list arrow navigation while a filter is active.
        ...
        if (nextItem) {
           ...
           if (e.shiftKey) {
             ...
             // Arrow focus snaps selection to the new item (Settings â†’ selectAllOnItemChange reinforces this).
  ...
  const renderPane = (pane: PaneState, index: number) => {
    ...
    const fileListLayoutKey = `${computedViewMode}:${gridIconSz}:${listIconSz}:${gridMetrics.rowHeight}:${gridMetrics.minWidth}:${listMetrics.rowHeight}`;
    ...
    // Settings â†’ Fonts â†’ List row height is authoritative when set; auto path stays snug.
    ...
    const normPanePath = normalizePanePath(panePath);
    const isImmersiveWorkspace = isBndzImmersiveWorkspacePath(normPanePath);
    const isThisPc = normPanePath === '/' || normPanePath === '/this-pc';
    const isLibraries = normPanePath.toLowerCase() === '/shell:libraries';
    ...
    const openEntity = (entity: any) => {
      ...
      // Non-folder shell applets: ShellExecute â€” never fake a folder navigate.
      ...
      // Shortcuts / .lnk / .url â€” resolve and navigate or ShellExecute (never Quick Preview the link blob).
      const entityExt = String(entity.extension || entity.name?.split('.').pop() || '')
        .toLowerCase()
        .replace(/^\./, '');
      const attrs = Array.isArray(entity.attributes) ? entity.attributes as string[] : [];
      const isReparseFile = entity.type !== 'directory' && attrs.includes('reparse');
      if (!shellParsing && (entityExt === 'lnk' || entityExt === 'url' || isReparseFile)) {
        void import('../lib/ipcBridge').then(async ({ IPC }) => {
          IPC.recordPathOpen(winPath);
          if (entityExt === 'lnk' || entityExt === 'url') {
            try {
              const resolved = await IPC.resolveShortcut(winPath);
              if (resolved.success && resolved.isUrl) {
                IPC.executeContextMenuVerb(winPath, 'open');
                return;
              }
              if (resolved.success && resolved.targetExists && resolved.targetIsDirectory && resolved.targetPath) {
                setCurrentPath(toPanePath(resolved.targetPath), pane.id);
                if (isGlobal) {
                  setFilterText('');
                  omniFilterRef.current?.blur();
                }
          ...
          // File-target shortcut, broken link, or file symlink â€” let Explorer shell open it.
          IPC.executeContextMenuVerb(winPath, 'open');
        });
        return;
      }
      ...
      if (entity.type === 'directory') {
        ...
        // Default open = ShellExecute (Explorer association). Quick Look stays on Space / explicit.
        // Close any floating preview so activate never races a hangy binary preview.
        if (quickPreviewOpenRef.current) {
          setQuickPreviewOpen(false);
          setHomeQuickPreview(null);
        }
        import('../lib/ipcBridge').then(({ IPC }) => {
        ...
        if (openAllowed) {
          if (openOnIconOnly) {
            ...
            if (!t.closest('.bndz-clipboard-icon-slot, .bndz-list-thumb, img, canvas')) {
              // Select only â€” open requires an icon hit.
            } else {
              openEntity(entity);
              return;
            }
    ...
    /** Right-click selection only â€” never open/navigate (avoids dual-pane surprise opens). */
    ...
    const handleEntityDoubleClicked = (entity: any) => {
      ...
      // Arm suppress before navigate/ShellExecute so the trailing native dblclick is ignored.
    ...
    const handleEntityMiddleClick = (e: React.MouseEvent, entity: any) => {
      ...
      e.preventDefault();
      // Settings â†’ On middle mouse down (thumbnail/icons): folder contents peek when enabled.
      ...
      // Settings â†’ Enable blow ups on file icons as well â†’ middle-click Quick Look any file.
      if (config.enableBlowUpsOnFileIconsAsWell && entity.type !== 'directory') {
        const ext = String(entity.extension || entity.name?.split('.').pop() || '').toLowerCase().replace(/^\./, '');
        if (isShellActivateExt(ext)) return;
    ...
    const renderDetailColumn = (
      ...
      const metaColClass = `bndz-list-select-cell bndz-list-col-muted px-2 whitespace-nowrap overflow-hidden text-ellipsis ${mutedColClass}`;
      // Settings â†’ Use empty cell defaults
      const emptyDefault = (value: React.ReactNode): React.ReactNode => {
        if (value !== null && value !== undefined && value !== '') return value;
        return config.useEmptyCellDefaults ? <span className="text-gray-600">â€”</span> : null;
      };
      ...
      switch (colId) {
        ...
        case 'type':
          return (
            <div key={colId} className={metaColClass}>
        ...
        case 'size': {
          ...
          return (
            <div
              key={colId}
              className={`bndz-list-size-cell text-right flex justify-end items-center gap-1.5 min-w-0 ${metaColClass}`}
              title={sizeLabel}
            >
              <span className="bndz-list-size-label tabular-nums shrink-0">{sizeLabel}</span>
              {barPct > 0 && (
                <SizeBar
                  percent={barPct}
                  isDir={isDir}
        ...
        case 'attributes':
          return (
            <div key={colId} className={`${metaColClass} font-mono text-[10px] tracking-wider`} title={(entity.attributes || []).join(', ')}>
        ...
        case 'comment':
          return (
            <div key={colId} className={`${metaColClass} text-[11px] italic`} title={(entity as any).comment || ''}>
        ...
        case 'path': {
          ...
          return (
            <div key={colId} className={`${metaColClass} font-mono text-[10px]`} title={shown}>
              {shown}
            </div>
          );
        }
        case 'originalLocation': {
          ...
          return (
            <div key={colId} className={`${metaColClass} font-mono text-[10px]`} title={loc}>
              {loc || <span className="text-gray-600">â€”</span>}
            </div>
          );
        }
        case 'originalPath': {
          ...
          return (
            <div key={colId} className={`${metaColClass} font-mono text-[10px]`} title={op}>
              {op || <span className="text-gray-600">â€”</span>}
            </div>
          );
        }
        case 'ghostState': {
        ...
        case 'coldTarget': {
          ...
          return (
            <div key={colId} className={`${metaColClass} font-mono text-[10px]`} title={target}>
              {target || <span className="text-gray-600">â€”</span>}
            </div>
          );
        }
        case 'ramZone': {
          ...
          return (
            <div key={colId} className={metaColClass} title={label || undefined}>
              {label ? (
    ...
    const buildDragPaths = (anchorEntityId: string, selectionOverride?: string[]): string[] => {
      ...
      const pathForEntity = (se: any) => resolveEntityDragPath(se, panePath);
      let paths: string[];
      if (selection.includes(anchorEntityId) && selection.length > 0) {
        paths = selection.map((sid: string) => {
          ...
          if (!se) return null;
          return pathForEntity(se);
        }).filter(Boolean) as string[];
      } else {
        paths = [pathForEntity(anchorEntity)];
      }
      return paths.filter(p => isValidOutboundDragPath(p));
    };
    ...
    const buildFluidDragItems = (anchorEntityId: string, selectionOverride?: string[]) => {
      ...
      return ids.map((sid: string) => {
        const se = contents?.find((c: any) => c.id === sid);
        if (!se) return null;
        const p = resolveEntityDragPath(se, panePath);
    ...
    return (
      ...
      >
        ...
        {/* Tab Strip â€” dnd-kit horizontal reorder (same pattern as column headers) */}
        <PaneTabStrip
          paneId={pane.id}
          tabs={pane.tabs}
        ...
        {/* Breadcrumb Row â€” Files NavigationToolbar-height address strip under filesHost */}
        ...
        <div className={`bndz-files-address-strip flex ${config.applyColors ? '' : 'bg-[#1a1a1a]'} border-b border-[#333] items-center px-1.5 shrink-0 ${isDualPane && !isActive ? 'opacity-90' : ''}`}
            ...
            <div 
              ...
              title="Click to edit path Â· double-click empty for Command Hub"
              onClick={() => {
                 if (!isGlobal) {
              ...
              {editingAddressBarPaneId === pane.id && !isGlobal ? (
                 ...
                 <input 
                    ...
                    onKeyDown={async e => {
                        ...
                        if (e.key === 'Enter') {
                            ...
                            import('../lib/ipcBridge').then(async ({ IPC }) => {
                              const expand = (p: string) => IPC.expandEnvironmentPath(p);
                              // Settings â†’ Honor relative paths
                 ...
                 <AddressAutocompleteDropdown
                   ...
                   onSelect={item => {
                     ...
                     setEditingAddressBarPaneId(null);
                     // Settings â†’ Select match on drop down: highlight same-named item in the opened folder.
            ...
            />
            {/* View modes always visible; Group/density sit beside them (never clip the strip). */}
            <div className="bndz-list-views-cluster flex items-center shrink-0 gap-1.5 ml-0.5 mr-0.5">
              <div
                className="bndz-view-mode-strip flex bg-[#222] border border-[#444] rounded-[var(--bndz-radius-sm)] items-center p-[2px] text-[11px] shrink-0 gap-[2px]"
                role="toolbar"
                aria-label="View mode"
              >
                 <button type="button" onMouseDown={e => runWebViewPrimaryAction(e, () => setViewMode('details', pane.id))} className={`bndz-view-mode-btn bndz-view-mode-btn--details w-6 h-6 flex items-center justify-center ${currentTab.viewMode === 'details' ? 'bndz-view-mode-btn--active' : 'text-gray-300 hover:bg-[#333]'}`} title="Details View (click again for default)">
                     <img src={launcherIconUrl('view_details')} alt="" className="w-3.5 h-3.5 object-contain pointer-events-none" draggable={false} />
                 </button>
                 <button type="button" onMouseDown={e => runWebViewPrimaryAction(e, () => setViewMode('grid', pane.id))} className={`bndz-view-mode-btn bndz-view-mode-btn--grid w-6 h-6 flex items-center justify-center ${currentTab.viewMode === 'grid' ? 'bndz-view-mode-btn--active' : 'text-gray-300 hover:bg-[#333]'}`} title="Grid View (click again for default)">
                     <img src={launcherIconUrl('view_grid')} alt="" className="w-3.5 h-3.5 object-contain pointer-events-none" draggable={false} />
                 </button>
                 <button type="button" onMouseDown={e => runWebViewPrimaryAction(e, () => setViewMode('list', pane.id))} className={`bndz-view-mode-btn bndz-view-mode-btn--list w-6 h-6 flex items-center justify-center ${currentTab.viewMode === 'list' ? 'bndz-view-mode-btn--active' : 'text-gray-300 hover:bg-[#333]'}`} title="List View (click again for default)">
                     <img src={launcherIconUrl('view_list')} alt="" className="w-3.5 h-3.5 object-contain pointer-events-none" draggable={false} />
                 </button>
                 <button type="button" onMouseDown={e => runWebViewPrimaryAction(e, () => setViewMode('columns', pane.id))} className={`bndz-view-mode-btn bndz-view-mode-btn--columns w-6 h-6 flex items-center justify-center ${currentTab.viewMode === 'columns' ? 'bndz-view-mode-btn--active' : 'text-gray-300 hover:bg-[#333]'}`} title="Columns View (click again for default)">
                     <img src={launcherIconUrl('view_columns')} alt="" className="w-3.5 h-3.5 object-contain pointer-events-none" draggable={false} />
                 </button>
                 <button type="button" onMouseDown={e => runWebViewPrimaryAction(e, () => setViewMode('size', pane.id))} className={`bndz-view-mode-btn bndz-view-mode-btn--size w-6 h-6 flex items-center justify-center ${currentTab.viewMode === 'size' ? 'bndz-view-mode-btn--active' : 'text-gray-300 hover:bg-[#333]'}`} title="Size map">
                     <img src={launcherIconUrl('folder_size_sync')} alt="" className="w-3.5 h-3.5 object-contain pointer-events-none" draggable={false} />
                 </button>
              </div>
              <div className="bndz-view-extras flex items-center gap-1 shrink-0 min-w-[108px] justify-end">
              ...
              {(computedViewMode === 'grid' || computedViewMode === 'list') && (
                <BndzDensitySlider
                  ...
                  title={computedViewMode === 'grid' ? 'Grid icon size' : 'List density (narrow â†’ wide columns)'}
                />
              )}
              </div>
            </div>
        ...
        {!isImmersiveWorkspace && (computedViewMode === 'details') && (settingsRt.list.showSortHeaders || config.showSortHeadersInAllViews !== false) && (
        <div
        ...
        {!!config.showFilterInformationInList && isActive && !isGlobal && !isFindingTabActive && (
          ...
          <div className="bndz-list-filter-info shrink-0 flex items-center gap-2 px-3 py-1 text-[11px] text-sky-200/90 bg-sky-500/[0.07] border-b border-sky-500/15">
            ...
            <span className="truncate">
              Filter
              {filterText.trim() && !filterText.trimStart().startsWith('> ') ? `: â€œ${filterText.trim()}â€` : ''}
              {pane.filterRegex?.trim() ? ` Â· regex /${pane.filterRegex.trim()}/` : ''}
              {contents ? ` Â· ${contents.length} match(es)` : ''}
            </span>
            <button
              type="button"
        ...
        {!!config.showSearchInformationInList && (isGlobal || isFindingTabActive) && (
          <div className="bndz-list-search-info shrink-0 flex items-center gap-2 px-3 py-1 text-[11px] text-amber-200/90 bg-amber-500/[0.07] border-b border-amber-500/15">
            ...
            <span className="truncate">
              {isFindingTabActive
                ? `Finding: â€œ${currentTab.findingQuery || ''}â€`
                : `Search: â€œ${filterText.trimStart().replace(/^>\s*/, '')}â€`}
              {contents ? ` Â· ${contents.length} result(s)` : ''}
              {isGlobal && globalSearchEngine ? ` Â· ${globalSearchEngine}` : ''}
            </span>
          </div>
        )}
        ...
        {!isImmersiveWorkspace && !isGlobal && !isFindingTabActive && (
          <ListFilterChips
        ...
        <div 
           ...
           style={{
             ...
             // Settings â†’ Mouse â†’ Scroll margin: keep focused/selected rows clear of edges
           ...
           onPointerDownCapture={(e) => {
              ...
              const buildSelectMeta = (): MarqueeSelectMeta | undefined => {
                ...
                // Always use arithmetic hit-test for non-empty lists â€” O(n) AABB vs O(nÃ—5Ã—elementsFromPoint).
              ...
              // Multi-signal hit test (listGestureHit): gutters / empty / row chrome â†’ marquee;
              // icon, name, column cells â†’ item select+drag. Never convert item presses to marquee.
              ...
              if (gestureIntent === 'marquee') {
                if (mouseRt.enableSurroundSelection === false) {
                  // Surround selection off â€” empty-canvas press only clears selection.
                  setSelectedItems([], pane.id);
                  scheduleSelectionChrome([], true);
                  scheduleQuickActionsBar(false);
                  return;
              ...
              // Item hit â†’ select / drag / double-click.
              if (!rowEl) return;
              ...
              // Settings â†’ Allow dragging items by the thumbnail (default off: icon press does not arm drag).
              const hitTarget = e.target as HTMLElement;
              const inListIconWell = !!hitTarget.closest('.bndz-list-icon-well');
              const hitOnThumb = !inListIconWell && !!hitTarget.closest('.bndz-clipboard-icon-slot, img, canvas');
              const thumbDragOk = mouseRt.dragByThumbnail
                || !!config.allowDraggingItemsByTheThumbnail
                || inListIconWell
                // Native shell: Explorer-parity â€” icon column is a valid drag handle.
                || IPC.isNative;
              // Settings â†’ Allow dragging from a background window (inactive / unfocused host).
              const bgDragOk = mouseRt.allowDraggingFromBackground
                || !!config.allowDraggingFromABackgroundWindow
                || document.hasFocus()
                || pane.id === activePaneId;
              const mayArmDragFinal = mayArmDrag && (thumbDragOk || !hitOnThumb) && bgDragOk;
              if (!mayArmDrag && disallowListDrag) {
                IPC.postOleDndDebug({ kind: 'drag-blocked', reason: 'disallow-list-setting' });
              } else if (mayArmDrag && !mayArmDragFinal) {
                IPC.postOleDndDebug({
                  kind: 'drag-blocked',
                  thumb: hitOnThumb && !thumbDragOk,
                  bg: !bgDragOk,
                  focus: document.hasFocus(),
                  activePane: pane.id === activePaneId,
                });
              } else if (mayArmDragFinal) {
                const pressPaths = buildDragPaths(entityId, wasSelected || ctrlKey || shiftKey
                  ? [...new Set([...currentTab.selectedItems, entityId])]
                  : [entityId]);
                IPC.postOleDndDebug({
                  kind: 'list-press-arm',
                  entityId,
                  pathCount: pressPaths.length,
                  sample: pressPaths.slice(0, 2),
                });
              }
              ...
              // Select immediately on press â€” Explorer feel; don't wait for pointerup/WebView2 onClick.
              if (!ctrlKey && !shiftKey) {
                if (!wasSelected) {
                  setSelectedItems([entityId], pane.id);
                  scheduleSelectionChrome([entityId], true);
              ...
              if (mayArmDragFinal) {
                const dragDelay = IPC.isNative
                  ? (wasSelected ? DRAG_DELAY_NATIVE_SELECTED : DRAG_DELAY_NATIVE)
              ...
              let outboundHandoffAttempted = false;
              let boundaryPrevClientX = startX;
              let boundaryPrevClientY = startY;
              ...
              const syncDragModifiers = (e: { ctrlKey?: boolean; metaKey?: boolean; altKey?: boolean }) => {
                ...
                // Guard: modifier unchanged â€” no-op to prevent spurious React commits on every move.
              ...
              // Throttle drop-target hit-tests â€” only re-run when pointer moves > 2px.
              ...
              const onMove = (ev: PointerEvent) => {
                ...
                if (listGestureRef.current.mode === 'pending') {
                  ...
                  const CHROME_SNAP_SELECTORS = [
                    '.bndz-chrome-sidebar', '.bndz-chrome-tabs', '.bndz-chrome-toolbar',
                    '.bndz-chrome-menubar', '.bndz-chrome-plugins', '.bndz-chrome-preview',
                    '.bndz-chrome-statusbar', '[data-breadcrumb-path]',
                    '[data-nav-path]', '.bndz-archive-root', '.sidebar-pin-row',
                    '[data-new-tab-zone]', '[data-bndz-workspace-surface]',
                    '[data-mesh-drop-inbox]', '[data-drop-stack-zone]',
                  ];
                  chromeCacheRects = [];
                  ...
                  const localRawPaths = dragPaths.filter(p => !isMeshPath(p));
                  const dragSession = {
                    paths: dragPaths,
                    op: (copyDrag ? 'copy' : 'move') as 'copy' | 'move',
                  ...
                  if (localRawPaths.length) {
                    ...
                    // Do not START_DRAG at threshold â€” keep React/fluid ghosts + in-app cancel.
                    // OLE begins at window boundary (no ReleaseCapture).
                  } else {
                    IPC.postOleDndDebug({
                      kind: 'list-drag-no-local-paths',
                      totalCount: dragPaths.length,
                      sample: dragPaths.slice(0, 2),
                    });
                  }
                  IPC.postOleDndDebug({
                    kind: 'list-drag-mode',
                    localCount: localRawPaths.length,
                    totalCount: dragPaths.length,
                  ...
                  bindKeyModifiers();
                  // Do NOT setPointerCapture during file drag â€” WebView2 poisons
                ...
                if (listGestureRef.current.mode === 'drag') {
                  ...
                  // Outbound desktop handoff BEFORE ghost/hit-test work â€” otherwise the handoff
                  ...
                  if (
                    ...
                    outboundHandoffAttempted = true;
                    const session = getFileDragSession();
                    const handoffPaths = (session?.paths ?? [])
                      .map(p => toWindowsPath(p))
                      .filter(p => !isMeshPath(p) && isValidOutboundDragPath(p));
                    if (handoffPaths.length) {
                      ...
                      // Detach FE gesture immediately â€” do not wait for OLE_DRAG_ESCALATED.
                  ...
                  if (hitMoved) {
                    ...
                    // whether a folder target exists â€” avoids repeating the 5-probe elementsFromPoint
              ...
              const disarmHostOleIfIdle = () => {
                if (!oleDragStarted && !hostOleEscalated && !nativeOleDragRef.current && !outboundHandoffAttempted) {
                  IPC.notifyFileDragActive(false);
                }
              };

              const detachFeAfterBoundaryHandoff = () => {
                suppressRowClickRef.current = true;
                listGestureRef.current = null;
              ...
              const onUp = (ev: PointerEvent) => {
                ...
                if (gesture?.mode === 'drag' && (oleDragStarted || nativeOleDragRef.current)) {
                  suppressRowClickRef.current = true;
                  listGestureRef.current = null;
                  clearFileDragChrome();
                  clearListDragGhost({ immediate: true });
                ...
                if (gesture?.mode === 'drag' && outboundHandoffAttempted && !oleDragStarted) {
                  detachFeAfterBoundaryHandoff();
                  return;
                }

                if (gesture?.mode === 'drag' && !oleDragStarted) {
                  // Sync hover before drop resolution â€” WebView2 poisons elementsFromPoint on pointer-up.
                  applyFileDragHoverAtPoint(ev.clientX, ev.clientY);
                  ...
                  const recalledHover = recallPointerDragHover(ev.clientX, ev.clientY);
                  const navTreeTarget = hitTestNavTreeAtPoint(ev.clientX, ev.clientY);
                  const breadcrumbTarget = navTreeTarget ? null : hitTestBreadcrumbAtPoint(ev.clientX, ev.clientY);
                  ...
                  if (millerDropPath) {
                    ...
                    if (!dropEnt && recalledHover?.listFolderId) {
                      dropEnt = targetContents?.find((c: any) => c.id === recalledHover.listFolderId && c.type === 'directory') ?? null;
                    }
                    if (!dropEnt && dragTargetIdRef.current) {
                      dropEnt = targetContents?.find((c: any) => c.id === dragTargetIdRef.current && c.type === 'directory') ?? null;
                    }
                  }
                  ...
                  const meshDropInbox = hitTestClosestAtPoint(ev.clientX, ev.clientY, '[data-mesh-drop-inbox]');
                  ...
                  const dropStackHit = hitTestClosestAtPoint(ev.clientX, ev.clientY, '[data-drop-stack-zone], [data-plugin-tab-id="dropstack"]');
                  ...
                  if (dragPaths.length) {
                    ...
                    const destCanon = canonicalDropPath(destPath);
                    const crossMeshLocal = isMeshPath(destCanon) !== dragPaths.some(isMeshPath);
                    const op = resolveDropOperation({
                      payloadCopy: gesture.copyDrag,
                      dropModifierCopy: dropModifierRef.current.copy,
                      ctrlKey: ev.ctrlKey || ev.metaKey,
                    ...
                    const hasForeignTarget = crossMeshLocal || !!(
                      navTreeTarget
                      || breadcrumbTarget
                      || hover
                      || dropEnt
                    ...
                    const explicitDropTarget = !!(navTreeTarget || breadcrumbTarget);
                    {
                      ...
                      if (shouldCommitInternalFileDrop(dropOpts)) {
                        // Settings â†’ Native drag and drop context menu (Copy / Move / Cancel at drop).
                  ...
                  setDragTargetHighlight(null);
                  suppressRowClickRef.current = true;
                  window.dispatchEvent(new CustomEvent('bndz-suppress-tree-click'));
                  internalDragRef.current = false;
                  clearFileDragChrome();
                  clearListDragGhost();
                  endFileDragSession();
                  ...
                  if (isDoubleTap && gesture.entityId) {
                    listEntityDblRef.current = null;
                    // Resolve entity from the OWNER pane listing â€” not this render's contents/path.
                    ...
                    if (entity) {
                      suppressRowClickRef.current = true;
                      // Must arm before open â€” native dblclick fires after this pointerup.
                      suppressNativeDblUntilRef.current = performance.now() + 700;
                  ...
                  // WebView2 often drops row onClick â€” commit click side-effects on pointerup.
                ...
                clearDragSession();
                if (!oleDragStarted && !hostOleEscalated && !nativeOleDragRef.current && !outboundHandoffAttempted) {
                  // Native outbound: boundary START_DRAG owns disarm â€” not pointer-up flicker.
                  const wasNativeFileDrag = IPC.isNative && gesture?.mode === 'drag';
                  if (!wasNativeFileDrag) IPC.notifyFileDragActive(false);
                }
              };

              const onCancel = (ev: PointerEvent) => {
                if (ev.pointerId !== capturePointerId) return;
                if (oleDragStarted || nativeOleDragRef.current || hostOleEscalated) return;
                const gest = listGestureRef.current;
                // WebView2 pointercancel at rim â€” START_DRAG while LMB may still be down.
                if (IPC.isNative && gest && hasMetDragThreshold() && gest.mode === 'drag') {
                  const session = getFileDragSession();
                  const handoffPaths = (session?.paths ?? [])
                    .map(p => toWindowsPath(p))
                    .filter(p => !isMeshPath(p) && isValidOutboundDragPath(p));
                  if (!outboundHandoffAttempted && handoffPaths.length) {
                    outboundHandoffAttempted = true;
                    performOutboundOleBoundaryHandoff({
                      paths: handoffPaths,
                      pointerId: capturePointerId,
        ...
        >
          ...
          {isPaneLoading && (
             <ListPaneSkeleton
               label={`Streaming ${formatUiPath(panePath) || panePath.split('/').pop() || panePath}â€¦`}
               rows={16}
             />
          )}
          {!isPaneLoading && isGlobal && (isGlobalSearchLoading || (isFindingTabActive && currentTab.findingLoading)) && (
             <div className="flex flex-col items-center justify-center h-full gap-3 text-amber-400/80 min-h-[200px]">
               ...
               <span className="text-[11px]">Searching{globalSearchEngine === 'indexed' ? ' local cache' : config.enableEverythingSearch !== false ? ' with Everything' : ''}â€¦</span>
             </div>
          )}
            ...
            <BndzHomeView
              ...
              onIndexInvite={() => {
                void IPC.reindexBndzDefaults().then(r => {
                  if (r?.ok) setToastMessage('Indexing default librariesâ€¦');
                ...
                <div className="bndz-list-empty-plaque text-gray-500">
                  {pathLoadErrors[normPanePath] && !(listRows?.length ?? 0) ? (
                    <>
                      ...
                      <span className="text-[12px] text-rose-200/90 max-w-md">
                        {/^IPC timeout:/i.test(pathLoadErrors[normPanePath])
                          ? 'Folder load timed out. The host may be busy â€” retry in a moment.'
                          : pathLoadErrors[normPanePath]}
                      </span>
                      <button
                        type="button"
                    ...
                    <BndzIndexEmptyState
                      ...
                      hint="Build or refresh the local search index so global search feels like Everything â€” without walking the whole disk."
                      onIndexed={() => {
                        void refetchPath(normPanePath);
                      }}
                    />
                      ...
                      <span className="text-[11px]">
                        {isFindingTabActive && currentTab.findingError ? currentTab.findingError
                          ...
                          : isGlobal && config.enableEverythingSearch === false
                            ? 'No results. Everything is off â€” enable it in Settings or build the BNDZ index.'
  ...
  const currentFolderListing = useMemo(() => {
    ...
    // This PC / Libraries synthesize list entities â€” they are not in pathContentsCache.
  ...
  const previewEntity = useMemo(() => {
    const selectionSource = (marquee && marqueePreviewIds && marqueePreviewIds.length > 0)
      ? marqueePreviewIds
      : currentTab.selectedItems;
    const anchorId =
      (focusedItemId && selectionSource.includes(focusedItemId))
        ? focusedItemId
        : (selectionSource[0] || null);
    if (anchorId) {
      const fromSel = resolveInCurrentFolder(anchorId) || getResolvedEntity(anchorId);
      if (fromSel) return fromSel;
    }
    if (anchorId?.startsWith('drive-') || anchorId?.startsWith('loc:')) {
      const syn = getResolvedEntity(anchorId);
      if (syn) return syn;
    }
    if (selectionSource.length > 0) {
      // Selection exists but entity not resolved yet â€” do not fall back to stale focus/location.
      return null;
    }
    const fromFocus = resolveInCurrentFolder(focusedItemId) || getResolvedEntity(focusedItemId);
    if (fromFocus) return fromFocus;
    return getLocationEntityFromPath(currentTab.path);
  }, [marquee, marqueePreviewIds, currentTab.selectedItems, currentTab.path, focusedItemId, currentFolderListing, navigationDrives, drives, pathContentsCache]);
  ...
  const quickPreviewItems = useMemo(() => {
    ...
    // With 0 or 1 selected (the common case â€” Space on a single file), browse the
  ...
  const openQuickPreview = React.useCallback((startIndex?: number) => {
    ...
    // Apps / installers: ShellExecute only â€” QL + PE metadata hangs the UI.
    if (item.entity?.type !== 'directory' && isShellActivateExt(ext)) {
      setToastMessage('Use double-click to launch apps â€” Quick Look skips executables.');
      return;
    }
    if (item?.path) {
      // Video still pauses/stashes into the floating player. Audio uses a shared
      // decoder â€” leave it playing so Space pop-out never glitches playback.
  ...
  // Do NOT dump raw colorConfig JSON onto this node â€” that clobbered gradients and live preview.
  ...
  const handleCommandDeckTool = React.useCallback((id: ContextToolId) => {
    switch (id) {
      ...
      case 'mesh-drop':
        ...
        setShowMeshDropDialog(true);
        openBottomPlugin('remote-mesh', { tab: 'drop' });
        break;
      ...
      case 'mesh-download': {
        ...
        void (async () => {
          const dest = await IPC.openFolderDialog('Download Mesh files toâ€¦');
          if (!dest) return;
          const res = await meshDownloadSelection(meshSel, dest);
          setToastMessage(res.ok ? 'Mesh download startedâ€¦' : (res.error || 'Download failed'), res.ok ? 'success' : 'warning');
        })();
        break;
      }
      case 'mesh-edit-remote': {
        ...
        void (async () => {
          try {
            ...
            setToastMessage('Remote file hydrated â€” save writes back to the host.', 'success');
          } catch (e: any) {
      ...
      case 'mesh-ephemeral':
        ...
        setToastMessage('Remote Â· Ephemeral (Incus)', 'info');
        break;
      case 'archive-extract': {
        const archivePath = bottomSelectionTargets.paths[0];
        if (!archivePath) break;
        void (async () => {
          const dest = await IPC.openFolderDialog('Extract archive toâ€¦');
          if (!dest) return;
          const res = await IPC.extractArchive(toWindowsPath(archivePath), dest);
          if (isQueuedIpcResult(res)) {
            setToastMessage('Extract queued â€” see transfer panel.', 'info');
            return;
          }
      ...
      case 'index-folder': {
        ...
        void IPC.indexBndzLocation(folderPath).then(res => {
          setToastMessage(
            res.ok ? 'Indexing folder for BNDZ searchâ€¦' : (res.error || 'Indexing failed.'),
      ...
      case 'analyze-audio': {
        ...
        setToastMessage(`Analyzing ${audioPaths.length} audio file${audioPaths.length > 1 ? 's' : ''}â€¦`, 'info');
        void IPC.analyzeMusicBatch(audioPaths.map(p => toWindowsPath(p)), true).then(res => {
          if (res.ok) {
            setToastMessage(
              `Analyzed ${res.analyzed ?? audioPaths.length} file${(res.analyzed ?? audioPaths.length) > 1 ? 's' : ''} â€” BPM + Key written`,
              'success',
            );
      ...
      case 'work-intent': {
        ...
        // Only open when pack resolved an installed plugin â€” never toast-spam missing ones.
  ...
  const activeToolbarProfileRaw = config.toolbarProfiles?.[config.activeToolbarProfileIndex || 0];
  const activeToolbarProfile = Array.isArray(activeToolbarProfileRaw) ? activeToolbarProfileRaw : [];
  ...
  return (
    ...
    >
      ...
      <div
        ...
        onPointerDown={isNativeShellHostBoot() ? undefined : (e) => {
          e.stopPropagation();
          if ((e.target as HTMLElement).closest('[data-window-btn],[data-menu-trigger]')) return;
          if (e.button === 0) IPC.windowChrome('drag');
        }}
        onMouseDown={isNativeShellHostBoot() ? undefined : (e) => {
          e.stopPropagation();
          if ((e.target as HTMLElement).closest('[data-window-btn],[data-menu-trigger]')) return;
        }}
        onDoubleClick={isNativeShellHostBoot() ? undefined : () => {
      ...
      >
         ...
         <div
           ...
           onMouseMove={(e) => {
             ...
             if (id && id !== openMenuId) {
               menubarOpenedByHoverRef.current = true;
               setOpenMenuId(id);
             }
           }}
         >
             ...
             {config.enableSubmenus !== false && config.enableContextSubmenus !== false && (
                 <MenubarPortalMenu open={openMenuId === 'File'} anchorEl={menubarAnchors.current['File']} minWidth={260}>
                    ...
                    <MenubarSubmenu label="Open Special" iconId="folder_open_ui">
                      ...
                      })}><Icons8Icon id="folder_open_ui" size={14} /> Openâ€¦ <span className="ml-auto text-[10px] text-gray-500">Ctrl+O</span></div>
                      <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2" onMouseDown={menuAct(() => runShellVerbOnSelection('openas'))}>
                        <Icons8Icon id="folder_open_ui" size={14} /> Open withâ€¦ <span className="ml-auto text-[10px] text-gray-500">Ctrl+Alt+Enter</span>
                      </div>
                      <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2" onMouseDown={menuAct(() => {
                        void (async () => {
                      ...
                      })}><Icons8Icon id="cmd" size={14} /> Open with Argumentsâ€¦</div>
                      <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2" onMouseDown={menuAct(() => {
                    ...
                    <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2" onMouseDown={menuAct(() => openBottomPlugin('batch-rename'))}><Icons8Icon id="batch_rename" size={14} /> Batch / Smart Renameâ€¦</div>
                    <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2" onMouseDown={menuAct(() => {
                    ...
                    <MenubarSubmenu label="To Clipboard" iconId="clipboard">
                      ...
                      <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2" onMouseDown={menuAct(() => { void duplicateSelectedItems(); })}><Icons8Icon id="copy" size={14} /> Duplicate</div>
                    </MenubarSubmenu>
                    ...
                    <MenubarSubmenu label="Recent Files">
                      {((config.recentFiles as string[] | undefined) || []).length === 0 ? (
                        ...
                        ((config.recentFiles as string[]) || []).map(rp => (
                          <div
                            key={rp}
                            className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 truncate max-w-[320px]"
                            title={rp}
                            onMouseDown={menuAct(() => setCurrentPath(rp))}
                          >
                            {rp.replace(/^\//, '').replace(/\//g, ' â€º ') || 'This PC'}
                          </div>
                        ))
                      )}
                    </MenubarSubmenu>
                    ...
                    <MenubarSubmenu label="New" iconId="new_folder">
                            ...
                            <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200" onMouseDown={menuAct(async () => {
                              const paths = getSelectedEntityPaths();
                              ...
                              setToastMessage(isQueuedIpcResult(res) ? 'Archive queued â€” see transfer panel.' : (res.ok ? 'Archive created.' : (res.error || 'Archive failed.')));
                            })}>ZIP Archive</div>
                            <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200" onMouseDown={menuAct(async () => {
                              const paths = getSelectedEntityPaths();
                              ...
                              setToastMessage(isQueuedIpcResult(res) ? 'Shortcut queued â€” see transfer panel.' : (res.success ? 'Shortcut created.' : (res.error || 'Failed to create shortcut.')));
                            })}>Shortcut</div>
                                ...
                                {fileMenuShellNewItems.map((item, i) => {
                                  ...
                                  return (
                                    <div
                                      ...
                                      className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200"
                                      onMouseDown={menuAct(() => {
                    ...
                    <MenubarSubmenu label="Copy / Move / Backup" iconId="copy_to">
                      <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2" onMouseDown={menuAct(() => { void copyOrMoveToTarget('copy'); })}><Icons8Icon id="copy_to" size={14} /> Copy Toâ€¦</div>
                      <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2" onMouseDown={menuAct(() => { void copyOrMoveToTarget('move'); })}><Icons8Icon id="cut" size={14} /> Move Toâ€¦</div>
                      <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2" onMouseDown={menuAct(() => {
                        const other = panes.find(p => p.id !== activePaneId);
                      ...
                      })}><Icons8Icon id="copy" size={14} /> Backup Toâ€¦ (copy)</div>
                    </MenubarSubmenu>
                    ...
                    <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2" onMouseDown={menuAct(() => refreshWorkspace())}><Icons8Icon id="refresh" size={14} /> Refresh</div>
                    <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2" onMouseDown={menuAct(() => {
                        const entities = getMenuSelectedEntities();
                        if (!entities.length) {
                    ...
                    <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2" onMouseDown={menuAct(() => openBottomPlugin('metadata'))}>
                    ...
                    <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2" onMouseDown={menuAct(() => runShellVerbOnSelection('share'))}><Icons8Icon id="share" size={14} /> Shareâ€¦</div>
                    <div className="h-[1px] bg-[#444] my-1"></div>
                    <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2" onMouseDown={menuAct(() => setIsConfigDialogOpen(true))}><Icons8Icon id="config" size={14} /> Configuration...</div>
                    <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2" onMouseDown={menuAct(() => setIsToolbarConfigOpen(true))}><Icons8Icon id="wrench" size={14} /> Customize Toolbar...</div>
                    <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2" onMouseDown={menuAct(async () => {
                    ...
                    })}><Icons8Icon id="download" size={14} /> Export Settingsâ€¦</div>
                    <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2" onMouseDown={menuAct(async () => {
                      const r = await import('../lib/settingsExport').then(m => m.importSettingsBundle(s => updateConfig(s as any)));
                      setToastMessage(r.message);
                    })}><Icons8Icon id="upload" size={14} /> Import Settingsâ€¦</div>
                    <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2" onMouseDown={menuAct(() => { void navigateToAppDataFolder('ini'); })}><Icons8Icon id="config" size={14} /> Open Configuration File</div>
                    <div className="h-[1px] bg-[#444] my-1"></div>
         ...
         <div className="relative shrink-0" ref={bindMenuAnchor('Edit')}>
             <button type="button" 
                 ...
                 onPointerDown={stopMenubarPointerBubble} onMouseDown={stopMenubarPointerBubble} onClick={openMenubarMenu('Edit')}             >Edit</button>
             {config.enableSubmenus !== false && config.enableContextSubmenus !== false && (
                 <MenubarPortalMenu open={openMenuId === 'Edit'} anchorEl={menubarAnchors.current['Edit']} minWidth={240}>
                    <MenubarSubmenu label="New">
                      ...
                      <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200" onMouseDown={menuAct(async () => {
                        ...
                        const target = toWindowsPath(paths[0]);
                        const res = await IPC.createLink(`${target}.lnk`, target, 'shortcut');
                        setToastMessage(isQueuedIpcResult(res) ? 'Shortcut queued â€” see transfer panel.' : (res.success ? 'Shortcut created.' : (res.error || 'Failed.')));
                      })}>New Shortcut</div>
                    </MenubarSubmenu>
                    <div className="h-[1px] bg-[#444] my-1"></div>
                    <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2" onMouseDown={menuAct(() => {
                    ...
                    <MenubarSubmenu label="Paste Special">
                      {(() => {
                        ...
                        const fileRow = (label: React.ReactNode, fn: () => void | Promise<void>, shortcut?: string) => (
                          <div
                            key={String(label)}
                            className={fileClip ? 'px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2' : 'px-3 py-1 text-sm text-gray-500 cursor-default select-none'}
                            onMouseDown={act(fn)}
                          >
                            {label}
                        ...
                        return (
                          <>
                            {fileRow('Paste Here to New Subfolderâ€¦', () => { void pasteIntoNewSubfolder(); }, 'Ctrl+Shift+V')}
                            {fileRow('Paste Here with Pathâ€¦', () => {
                              void pasteIntoActivePane({ recreateSourceStructure: true });
                            })}
                            {fileRow('Paste Here Asâ€¦', () => { void pasteHereAs(); })}
                            <div className="h-[1px] bg-[#444] my-1" />
                            ...
                            <div className="h-[1px] bg-[#444] my-1" />
                            {/* Link paste always live â€” pasteAsLinksFromClipboard falls back to CF_HDROP when BNDZ clipboard is empty */}
                            <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2" onMouseDown={actAlways(() => { void pasteAsLinksFromClipboard('shortcut'); })}>Paste As Shortcut(s)</div>
                            <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2" onMouseDown={actAlways(() => { void pasteAsLinksFromClipboard('hardlink'); })}>Paste As Hard Link(s)</div>
                            {/* One symlink row â€” files AND folders; backend auto-detects file vs dir flag */}
                            <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2" onMouseDown={actAlways(() => { void pasteAsLinksFromClipboard('symlink'); })}>Paste As Symbolic Link(s)</div>
                            {/* Junction: folder targets only â€” always clickable; backend rejects file targets */}
                            <div
                              key="junction"
                              className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2"
                              title="NTFS junction â€” folder targets only"
                            ...
                            <div className="h-[1px] bg-[#444] my-1" />
                            <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2" onMouseDown={actAlways(() => { void pasteTextAsItems(); })}>Paste Text As Item(s)</div>
                            <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2" onMouseDown={actAlways(() => { void pasteTextIntoNewFile(); })}>
                              Paste Text Into New File <span className="ml-auto text-[10px] text-gray-500 pl-3">Ctrl+Alt+V</span>
                            </div>
                            <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2" onMouseDown={actAlways(() => { void pasteImageIntoNewPng(); })}>
                            ...
                            <div className="h-[1px] bg-[#444] my-1" />
                            {fileRow('Edit Clipboardâ€¦', () => { void editClipboardPaths(); })}
                            <div
                              className={clipboardHistory.length ? 'px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2' : 'px-3 py-1 text-sm text-gray-500 cursor-default select-none'}
                            ...
                            <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2" onMouseDown={actAlways(() => {
                              clearClipboard();
                              setToastMessage('Clipboard cleared.');
                            })}>Clear Clipboard</div>
                          </>
                    ...
                    <MenubarSubmenu label="Select">
                      ...
                      <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200" onMouseDown={menuAct(() => setSelectedItems([], activePaneId))}>Select None</div>
                      <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200" onMouseDown={menuAct(() => invertSelectionInActivePane())}>Invert Selection</div>
                      <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200" onMouseDown={menuAct(() => {
                    ...
                    <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2" onMouseDown={menuAct(() => { void runUndoRedo(false); })}>
                      <Icons8Icon id="undo" size={14} /> Undo <span className="ml-auto text-[10px] text-gray-500">Ctrl+Z</span>
                    </div>
                    <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2" onMouseDown={menuAct(() => { void runUndoRedo(true); })}>
                    ...
                    {config.logActionsAndEnableUndoRedo !== false && (
                      <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2" onMouseDown={menuAct(() => setShowHistoryDialog(true))}>
                        <Icons8Icon id="clock_ui" size={14} /> Historyâ€¦
                      </div>
                    )}
                    <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2" onMouseDown={menuAct(() => setShowHistoryDialog(true))}>
                      <Icons8Icon id="clock_ui" size={14} /> Action Log / Recent Opsâ€¦
                    </div>
                    <div className="h-[1px] bg-[#444] my-1"></div>
                    <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2" onMouseDown={menuAct(() => {
                      const ents = getMenuSelectedEntities();
                    ...
                    <MenubarSubmenu label="Copy To..." iconId="copy_to">
                      ...
                      <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200" onMouseDown={menuAct(() => { void copyOrMoveToTarget('copy'); })}>Browse...</div>
                    </MenubarSubmenu>
                    ...
                    <MenubarSubmenu label="Move To...">
                      ...
                      <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200" onMouseDown={menuAct(() => { void copyOrMoveToTarget('move'); })}>Browse...</div>
                    </MenubarSubmenu>

                    <MenubarSubmenu label="Backup Toâ€¦">
                      <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200" onMouseDown={menuAct(() => {
                        const other = panes.find(p => p.id !== activePaneId);
                        const dest = other?.tabs[other.activeTabIndex]?.path;
                        if (dest) void copyOrMoveToTarget('copy', dest);
                      ...
                      <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200" onMouseDown={menuAct(() => { void copyOrMoveToTarget('copy'); })}>Browseâ€¦</div>
                    </MenubarSubmenu>

                    <MenubarSubmenu label="Compare">
                      <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200" onMouseDown={menuAct(() => startFolderCompare())}>Compare / Sync Folders</div>
                      <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200" onMouseDown={menuAct(() => openBottomPlugin('find'))}>Find Differencesâ€¦</div>
                    </MenubarSubmenu>

                    <div className="h-[1px] bg-[#444] my-1"></div>
                    <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2" onMouseDown={menuAct(() => openBottomPlugin('batch-rename'))}>
                      <Icons8Icon id="sparkles_ui" size={14} /> Smart Rename
                    </div>
                    <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2" onMouseDown={menuAct(() => openBottomPlugin('find'))}>
                      <Icons8Icon id="file_search_ui" size={14} /> Find Filesâ€¦
                    </div>
                    <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2" onMouseDown={menuAct(() => { omniFilterRef.current?.focus(); })}>Find Now / Quick Search</div>
                    <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2" onMouseDown={menuAct(() => {
                      void (async () => {
                        const q = filterText.trim() || (await requestNativePrompt({
                          title: 'Global search',
         ...
         <div className="relative shrink-0" ref={bindMenuAnchor('View')}>
             ...
             {config.enableSubmenus !== false && config.enableContextSubmenus !== false && (
                 <MenubarPortalMenu open={openMenuId === 'View'} anchorEl={menubarAnchors.current['View']} minWidth={200}>
                    <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2" onMouseDown={menuAct(() => { toggleDualPane(); })}>
                       <Icons8Icon id="toggle_dual_pane" size={14} /> {isDualPane ? 'Single Pane' : 'Dual Pane'}
                    </div>
                    <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2" onMouseDown={menuAct(() => { togglePreviewPanel(); })}>
                        <Icons8Icon id="toggle_preview" size={14} /> {isPreviewPanelOpen ? 'Hide Preview Panel' : 'Show Preview Panel'}
                    </div>
                    <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2" onMouseDown={menuAct(() => { toggleBottomPanel(); })}>
                        <Icons8Icon id="toggle_bottom" size={14} /> {isBottomPanelOpen ? 'Hide Bottom Panel' : 'Show Bottom Panel'}
                    </div>
                    <div className="h-[1px] bg-[#444] my-1"></div>
                    <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2" onMouseDown={menuAct(() => { refreshWorkspace(); })}>
                       <Icons8Icon id="refresh" size={14} /> Refresh
                    </div>
                    <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2" onMouseDown={menuAct(() => scanCurrentFolderSizes(true, { manual: true }))}>
                       <Icons8Icon id="folder_size_sync" size={14} /> Get Folder Sizes
                    </div>
                    <div
                      className={`px-3 py-1 bndz-menubar-row cursor-pointer text-sm flex items-center gap-2 ${config.autoSyncFolderSizes !== false ? 'text-emerald-300' : 'text-gray-200'}`}
                      onMouseDown={menuAct(() => {
                    ...
                    >
                      <span className="w-4 text-center text-[11px]">{config.autoSyncFolderSizes !== false ? 'âœ“' : ''}</span>
                      <Icons8Icon id="folder_size_sync" size={14} className={config.autoSyncFolderSizes !== false ? '' : 'opacity-50'} />
                      Auto Sync Folder Sizes
                    </div>
                    <div className="h-[1px] bg-[#444] my-1"></div>
                    {/* Show hidden files toggle â€” synced with Settings â†’ showHiddenFiles */}
                    <div
                      className={`px-3 py-1 bndz-menubar-row cursor-pointer text-sm flex items-center gap-2 ${config.showHiddenFiles ? 'text-emerald-300' : 'text-gray-200'}`}
                      onMouseDown={menuAct(() => {
                    ...
                    >
                      <span className="w-4 text-center text-[11px] shrink-0">{config.showHiddenFiles ? 'âœ“' : ''}</span>
                      <Icons8Icon id="eye" size={14} className={config.showHiddenFiles ? '' : 'opacity-50'} />
                      Show Hidden Files
                    </div>
                    {/* Show system files toggle â€” synced with Settings â†’ showSystemFiles */}
                    <div
                      className={`px-3 py-1 bndz-menubar-row cursor-pointer text-sm flex items-center gap-2 ${config.showSystemFiles ? 'text-emerald-300' : 'text-gray-200'}`}
                      onMouseDown={menuAct(() => {
                    ...
                    >
                      <span className="w-4 text-center text-[11px] shrink-0">{config.showSystemFiles ? 'âœ“' : ''}</span>
                      <Icons8Icon id="filter" size={14} className={config.showSystemFiles ? '' : 'opacity-50'} />
                      Show System Files
                    </div>
                    <div className="h-[1px] bg-[#444] my-1"></div>
                    <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2" onMouseDown={menuAct(() => setViewMode('details', activePaneId))}><Icons8Icon id="view_details" size={14} /> Details</div>
                    <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2" onMouseDown={menuAct(() => setViewMode('grid', activePaneId))}><Icons8Icon id="view_grid" size={14} /> Grid</div>
                    <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2" onMouseDown={menuAct(() => setViewMode('list', activePaneId))}><Icons8Icon id="view_list" size={14} /> List</div>
                    <div className="h-[1px] bg-[#444] my-1"></div>
                    <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2" onMouseDown={menuAct(() => setIsConfigDialogOpen(true))}><Icons8Icon id="config" size={14} /> Configuration...</div>
                    <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2" onMouseDown={menuAct(() => setIsToolbarConfigOpen(true))}><Icons8Icon id="wrench" size={14} /> Customize Toolbar...</div>
                    <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2" onMouseDown={menuAct(() => setIsCommandPaletteOpen(true))}><Icons8Icon id="command_ui" size={14} /> Command Palette</div>
                    <div className="h-[1px] bg-[#444] my-1"></div>
                    <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2" onMouseDown={menuAct(() => { setShowTutorial(true); closeMenu(); })}><Icons8Icon id="sparkles_ui" size={14} /> Show tutorial</div>
                 </MenubarPortalMenu>
             )}
         </div>
         <div className="relative shrink-0" ref={bindMenuAnchor('Go')}>
             ...
             {config.enableSubmenus !== false && config.enableContextSubmenus !== false && (
                 <MenubarPortalMenu open={openMenuId === 'Go'} anchorEl={menubarAnchors.current['Go']} minWidth={260}>
                    <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2" onMouseDown={menuAct(() => goBack())}>
                       <Icons8Icon id="nav_back" size={14} /> Back
                    </div>
                    <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2" onMouseDown={menuAct(() => goForward())}>
                    ...
                    <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2" onMouseDown={menuAct(() => goUp())}>
                       <Icons8Icon id="nav_up" size={14} /> Up One Level
                    </div>
                    <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2" onMouseDown={menuAct(() => goToDriveRoot())}>Top (Drive Root)</div>
                    <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2" onMouseDown={menuAct(() => goUp())}>Up</div>
                    <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2" onMouseDown={menuAct(() => enterFocusedOrSelectedFolder())}>Down (Enter Folder)</div>
                    ...
                    <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2" onMouseDown={menuAct(() => setCurrentPath(homeTreePath))}>
                       <Icons8Icon id="home" size={14} /> {(windowsUsername && windowsUsername !== 'Public') ? windowsUsername : 'Profile'}
                    </div>
                    <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2" onMouseDown={menuAct(() => openWorkspaceToolTab(BNDZ_CANVAS))}>
                       <Icons8Icon id="view_grid" size={14} /> Spatial Canvas
                    </div>
                    <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2" onMouseDown={menuAct(() => openWorkspaceToolTab(BNDZ_AUTOMATION))}>
                       <Icons8Icon id="zap_ui" size={14} /> Automation
                    </div>
                    <div className="h-[1px] bg-[#444] my-1"></div>
                    <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2" onMouseDown={menuAct(() => setCurrentPath('/'))}>
                       <Icons8Icon id="this_pc" size={14} /> This PC
                    </div>
                    <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2" onMouseDown={menuAct(() => setCurrentPath('/shell:Desktop'))}><Icons8Icon id="monitor_ui" size={14} /> Desktop</div>
                    <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2" onMouseDown={menuAct(() => setCurrentPath('/shell:Personal'))}><Icons8Icon id="file_ui" size={14} /> Documents</div>
                    <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2" onMouseDown={menuAct(() => setCurrentPath('/shell:Downloads'))}><Icons8Icon id="arrow_down_circle_ui" size={14} /> Downloads</div>
                    ...
                    <MenubarSubmenu label="Recent Locations">
                      {((config.navigationHistory as any[]) || []).slice(0, 12).length === 0 ? (
                        ...
                        ((config.navigationHistory as any[]) || []).slice(0, 12).map((v: any, i: number) => {
                          ...
                          if (!p) return null;
                          return (
                            <div key={`${p}-${i}`} className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 truncate max-w-[320px]" title={p} onMouseDown={menuAct(() => setCurrentPath(p))}>
                    ...
                    <MenubarSubmenu label="Hotlist (Rapid access)">
                      {rapidAccessItems.length === 0 ? (
                        ...
                        rapidAccessItems.slice(0, 16).map((s) => (
                          <div key={s.path} className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 truncate max-w-[320px]" title={formatUiPath(s.path)} onMouseDown={menuAct(() => setCurrentPath(s.path))}>
                    ...
                    <MenubarSubmenu label="Tablist">
                      {currentPane?.tabs?.length ? currentPane.tabs.map((t: any, i: number) => (
                        <div key={t.id || i} className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 truncate max-w-[320px]" title={formatUiPath(t.path)} onMouseDown={menuAct(() => {
                    ...
                    <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2" onMouseDown={menuAct(() => setShowHistoryDialog(true))}>Historyâ€¦</div>
                    <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2" onMouseDown={menuAct(() => {
                      // Native FM: jump straight into the address bar â€” never browser prompt().
                      focusAddressBar();
                    })}>Go toâ€¦ / Go Now</div>
                    <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2" onMouseDown={menuAct(() => { void navigateToAppDataFolder('app'); })}>Application Folder</div>
                    <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2" onMouseDown={menuAct(() => { void navigateToAppDataFolder('appdata'); })}>AppData Folder</div>
                    <div className="h-[1px] bg-[#444] my-1"></div>
                    <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2" onMouseDown={menuAct(() => addTab(activePaneId, currentTab.path))}><Icons8Icon id="folder_open_ui" size={14} /> Open Location in New Tab</div>
                    <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2" onMouseDown={menuAct(() => {
                      void (async () => {
                        const q = filterText.trim() || (await requestNativePrompt({
                          title: 'New Finding Tab',
                    ...
                    })}><Icons8Icon id="file_search_ui" size={14} /> New Finding Tabâ€¦</div>
                 </MenubarPortalMenu>
             )}
         </div>
         <div className="relative shrink-0" ref={bindMenuAnchor('Tools')}>
             ...
             {config.enableSubmenus !== false && config.enableContextSubmenus !== false && (
                 <MenubarPortalMenu open={openMenuId === 'Tools'} anchorEl={menubarAnchors.current['Tools']} minWidth={260}>
                    ...
                    <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2" onMouseDown={menuAct(() => startFolderCompare())}>
                       <Icons8Icon id="sync_folders" size={14} /> Sync / Compare Folders
                    </div>
                    <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2" onMouseDown={menuAct(() => setIsSmartToolsOpen(true))}>
                    ...
                    <MenubarSubmenu label="Intent">
                      {WORK_INTENT_ORDER.map(id => {
                        ...
                        return (
                          <div
                            key={id}
                            className={`px-3 py-1 bndz-menubar-row cursor-pointer text-sm flex items-center gap-2 ${active ? 'text-amber-200' : 'text-gray-200'}`}
                            onMouseDown={menuAct(() => {
                    ...
                    <MenubarSubmenu label="Customize">
                      <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200" onMouseDown={menuAct(() => { void navigateToAppDataFolder('ini'); })}>Open Configuration File</div>
                      <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200" onMouseDown={menuAct(() => {
                        setConfigInitialTab('Keyboard Shortcuts');
                        setIsConfigDialogOpen(true);
                      })}>Keyboard Shortcutsâ€¦</div>
                      <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200" onMouseDown={menuAct(() => {
                        setConfigInitialTab('Refresh, Icons, History');
                        setIsConfigDialogOpen(true);
                      })}>File Iconsâ€¦</div>
                      <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200" onMouseDown={menuAct(() => setIsToolbarConfigOpen(true))}>Toolbarâ€¦</div>
                      <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200" onMouseDown={menuAct(() => {
                    ...
                    <div className="h-[1px] bg-[#444] my-1"></div>
                    <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2" onMouseDown={menuAct(() => setIsConfigDialogOpen(true))}>
         ...
         <div className="relative shrink-0" ref={bindMenuAnchor('Favorites')}>
             ...
             {config.enableSubmenus !== false && config.enableContextSubmenus !== false && (
                 <MenubarPortalMenu open={openMenuId === 'Favorites'} anchorEl={menubarAnchors.current['Favorites']} minWidth={260}>
                    <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2" onMouseDown={menuAct(() => { toggleFavoriteFolder(); })}><Icons8Icon id="zap_ui" size={14} /> Toggle Rapid access pin</div>
                    <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2" onMouseDown={menuAct(() => {
                      const path = collapseKnownFolderShadowPath(
                        resolveShellKnownFolderToFs(normalizePanePath(currentTab.path), shortcuts),
                        shortcuts,
                    ...
                    {rapidAccessItems.length === 0 ? (
                      ...
                      rapidAccessItems.map((s) => (
                        <div
                          key={s.path}
                          className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 truncate max-w-[340px] flex items-center gap-2"
                    ...
                    <MenubarSubmenu label="Special System Folders">
                      <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200" onMouseDown={menuAct(() => setCurrentPath('/shell:Desktop'))}>Desktop</div>
                      <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200" onMouseDown={menuAct(() => setCurrentPath('/shell:Personal'))}>Documents</div>
                      <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200" onMouseDown={menuAct(() => setCurrentPath('/shell:Downloads'))}>Downloads</div>
                      <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200" onMouseDown={menuAct(() => setCurrentPath('/shell:My Pictures'))}>Pictures</div>
                      <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200" onMouseDown={menuAct(() => setCurrentPath('/shell:My Music'))}>Music</div>
                      <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200" onMouseDown={menuAct(() => setCurrentPath('/shell:My Video'))}>Videos</div>
                      <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200" onMouseDown={menuAct(() => setCurrentPath('/shell:Libraries'))}>Libraries</div>
                      <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200" onMouseDown={menuAct(() => setCurrentPath(CONTROL_PANEL_PATH))}>Control Panel</div>
                      <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200" onMouseDown={menuAct(() => { void navigateToAppDataFolder('appdata'); })}>AppData</div>
                    </MenubarSubmenu>
                    <MenubarSubmenu label="Manage Rapid access">
                            <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200" onMouseDown={menuAct(() => setIsConfigDialogOpen(true))}>Organize pins...</div>
                            <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200" onMouseDown={menuAct(() => {
         ...
         <div className="relative shrink-0" ref={bindMenuAnchor('Tags')}>
             <button type="button" 
                 ...
                 onPointerDown={stopMenubarPointerBubble} onMouseDown={stopMenubarPointerBubble} onClick={openMenubarMenu('Tags')}             >Tags</button>
             {config.fileTaggingFeature !== false && config.enableSubmenus !== false && config.enableContextSubmenus !== false && (
                 <MenubarPortalMenu open={openMenuId === 'Tags'} anchorEl={menubarAnchors.current['Tags']} minWidth={240}>
                    <MenubarSubmenu label="Labels">
                      {availableTags.length === 0 ? (
                        ...
                        availableTags.map(tag => {
                          ...
                          return (
                            <div key={tag.name || tag.label} className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2"
                    ...
                    <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2" onMouseDown={menuAct(() => {
                      ...
                      if (!tag) { setToastMessage('No previous label â€” apply a tag first.'); return; }
                    ...
                    <div className="h-[1px] bg-[#444] my-1"></div>
                    <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2" onMouseDown={menuAct(() => { setTagAssignmentActive(true); })}>
                      <TagGlyph color="#9CA3AF" size={12} /> Tag assignment modeâ€¦
                    </div>
                    <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200 flex items-center gap-2" onMouseDown={menuAct(() => { setIsTagManagerOpen(true); })}>
         ...
         <div className="relative shrink-0" ref={bindMenuAnchor('Panes')}>
             ...
             {config.dualPaneFeature !== false && config.enableSubmenus !== false && config.enableContextSubmenus !== false && (
                 <MenubarPortalMenu open={openMenuId === 'Panes'} anchorEl={menubarAnchors.current['Panes']} minWidth={200}>
                   <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200" onMouseDown={menuAct(() => { toggleDualPane(); })}>Toggle Dual Pane</div>
                   <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200" onMouseDown={menuAct(() => { swapPanes(); })}>Swap Panes</div>
                   <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200" onMouseDown={menuAct(() => { syncPanesToSamePath(); })}>Sync Panes</div>
                   <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200" onMouseDown={menuAct(() => { setActivePaneId(panes[0]?.id || activePaneId); })}>Focus Left Pane</div>
                   <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200" onMouseDown={menuAct(() => { setActivePaneId(panes[1]?.id || activePaneId); })}>Focus Right Pane</div>
                   {isDualPane && (
                     <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200" onMouseDown={menuAct(() => { setDualPaneDiffActive(p => !p); })}>
         ...
         <div className="relative shrink-0" ref={bindMenuAnchor('Tabsets')}>
             ...
             {config.tabsets !== false && config.enableSubmenus !== false && config.enableContextSubmenus !== false && (
                 <MenubarPortalMenu open={openMenuId === 'Tabsets'} anchorEl={menubarAnchors.current['Tabsets']} minWidth={200}>
                    ...
                    <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200" onMouseDown={menuAct(() => setIsLoadTabsetOpen(true))}>Load Tabset...</div>
                 </MenubarPortalMenu>
             )}
         </div>
         <div className="relative shrink-0" ref={bindMenuAnchor('Window')}>
             ...
             {config.enableSubmenus !== false && config.enableContextSubmenus !== false && (
                 <MenubarPortalMenu open={openMenuId === 'Window'} anchorEl={menubarAnchors.current['Window']} minWidth={200}>
                    ...
                    <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200" onMouseDown={menuAct(() => { import('../lib/ipcBridge').then(({ IPC }) => IPC.windowChrome('minimize')); })}>Minimize</div>
                    <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200" onMouseDown={menuAct(() => { import('../lib/ipcBridge').then(({ IPC }) => IPC.windowChrome('maximize')); })}>Maximize / Restore</div>
         ...
         <div className="relative shrink-0" ref={bindMenuAnchor('Help')}>
             ...
             {config.enableSubmenus !== false && config.enableContextSubmenus !== false && (
                 <MenubarPortalMenu open={openMenuId === 'Help'} anchorEl={menubarAnchors.current['Help']} minWidth={220}>
                    <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200" onMouseDown={menuAct(() => setShowHelpTopics(true))}>Help Topics</div>
                    <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200" onMouseDown={menuAct(() => {
                      setConfigInitialTab('Keyboard Shortcuts');
                      setIsConfigDialogOpen(true);
                    })}>Keyboard Shortcuts</div>
                    <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200" onMouseDown={menuAct(() => setIsConfigDialogOpen(true))}>Settings Reference...</div>
                    <div className="h-[1px] bg-[#444] my-1"></div>
                    <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200" onMouseDown={menuAct(async () => {
                      await import('../lib/settingsExport').then(m => m.exportSettingsBundle(config as Record<string, unknown>));
                      setToastMessage('Settings exported.');
                    ...
                    <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200" onMouseDown={menuAct(() => {
                      setToastMessage('Checking for updatesâ€¦');
                    ...
                    <div className="h-[1px] bg-[#444] my-1"></div>
                    <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200" onMouseDown={menuAct(() => void IPC.openLegalDoc('eula').then(r => { if (!r.ok) setToastMessage(r.error || 'Could not open EULA.'); }))}>License Agreement (EULA)</div>
                    <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200" onMouseDown={menuAct(() => void IPC.openLegalDoc('privacy').then(r => { if (!r.ok) setToastMessage(r.error || 'Could not open Privacy Policy.'); }))}>Privacy Policy</div>
                    <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200" onMouseDown={menuAct(() => void IPC.openLegalDoc('third-party').then(r => { if (!r.ok) setToastMessage(r.error || 'Could not open third-party licenses.'); }))}>Third-Party Licenses</div>
                    <div className="px-3 py-1 bndz-menubar-row cursor-pointer text-sm text-gray-200" onMouseDown={menuAct(() => setShowAboutDialog(true))}>About BNDZ</div>
                 </MenubarPortalMenu>
             )}
         </div>
         </div>
         {/* Native-host: trailing flex gap â€” WinUI Caption owns window-drag here (never IPC BeginDrag). */}
         {isNativeShellHostBoot() && (
           <div
         ...
         {row.map((item: any, i: number) => {
             ...
             return (
                 <ToolbarButton 
                     ...
                     onContextMenu={(e) => {
                       // Settings â†’ Show last actions in toolbar button menu
      ...
      <div
        ...
        title="Filter Â· path Â· >command Â· ::script â€” double-click for Command Hub"
        onDoubleClick={(e) => {
          const t = e.target as HTMLElement;
          // Empty bar / chrome / icon â†’ Command Hub. Filled input keeps word-select.
      ...
      >
         ...
         <input 
            ...
            placeholder="Filter Â· path Â· >command Â· ::script â€” dbl-click for Hub"
            value={filterText}
            onChange={(e) => {
      ...
      <div className="flex flex-1 overflow-hidden min-h-0 relative">
         <ResizablePanelGroup
             key={`workspace-outer-v${config.workspaceLayoutVersion ?? WORKSPACE_LAYOUT_VERSION}-${previewDockedInWorkspace ? 'd' : 'c'}-${effectivePreviewOpen ? 'p' : 'n'}`}
             id="workspace-outer"
               ...
               <LeftSidebar
                     ...
                     >
                      ...
                      <div className="mx-3 my-2 px-3 py-3 text-center rounded-md border border-dashed border-[#333] bg-[#151515]/80">
                        <p className="text-[10px] text-gray-500 leading-relaxed">
                          {drivesReady ? 'No local drives detected.' : 'Scanning local drivesâ€¦'}
                        </p>
                        {drivesReady && (
                          <button
                            type="button"
                           ...
                           return (
                              <div 
                                 ...
                                 onPointerDown={(e) => {
                                   ...
                                   // Grip owns reorder â€” don't navigate from the drag handle.
                                 ...
                                 onClick={(e) => {
                                   ...
                                   e.stopPropagation();
                                   const target = collapseKnownFolderShadowPath(s.path, shortcuts);
                                   // Settings â†’ Open favorite files directly
                                   if (config.openFavoriteFilesDirectly) {
                                     const looksFile = /\.[A-Za-z0-9]{1,8}$/.test(target.split(/[/\\]/).pop() || '')
                                       && !target.toLowerCase().includes('/shell:');
                                     if (looksFile) {
                              ...
                              >
                                 ...
                                 {isRenaming ? (
                                   <input
                                      ...
                                      type="text"
                                      defaultValue={s.name}
                                      className="text-[11px] font-medium bndz-inline-rename-input rounded-[var(--bndz-radius-sm)] px-1 py-0 flex-1 min-w-0 outline-none"
                  ...
                  treeContent={
                    <VirtualizedNavTree
                      ...
                      onNavigate={(path) => {
                        if (isBndzCanvasPath(path) || isBndzAutomationPath(path)) {
                          openWorkspaceToolTab(path);
                          return;
                        }
                        // Same path as Drives / Rapid Access â€” clears suppressNavClickUntilRef
            ...
            >
               ...
               >
                  ...
                  >
                    {(() => {
                     ...
                     <div className="flex-1 flex flex-col h-full min-h-0 overflow-hidden bg-[#202020] relative">
                        ...
                        <div className="flex flex-col flex-1 overflow-hidden min-h-0">
                           ...
                           {/* DualPane DiffPlex compare strip â€” shown when diff mode is active */}
                    ...
                    >
                      ...
                      <ResizablePanel
                        ...
                        minSize={panelPct(PREVIEW_COLLAPSE_SIZE)}
                      ...
                      >
                        <div
                          ...
                          onContextMenu={(e) => {
                            if (previewEntity && previewPath) {
                              handleContextMenuRequest(
                                e,
                                currentPath,
                                previewEntity.id,
                                previewEntity.type === 'directory',
                                previewEntity.name,
                                undefined,
                                'preview'
                              );
                  ...
                  <ResizableHandle
                     ...
                     title="Drag to resize Â· Double-click to reset default height"
                     onDoubleClick={(e) => {
                       e.preventDefault();
                       e.stopPropagation();
                  ...
                  />
                  {/* Bottom Plugin Panel â€” always mounted; collapsed via panelRef when hidden */}
                  <ResizablePanel
                     id="bottom"
                  ...
                  >
                     ...
                     >
                        <BottomPluginPanel
                           ...
                           primarySelectedPath={bottomSelectionTargets.paths[0] ?? null}
      ...
      {/* Transfer queue (native background jobs) â€” sole live progress surface */}
      ...
      <div
        className="bndz-chrome-statusbar bndz-status-bar-tree-tab bndz-sidebar-section-header px-3 py-1.5 pl-4 flex items-center justify-between shrink-0 gap-3 min-h-[26px] text-[11px]"
           ...
           {getListIxBehavior(config).showDragStatusBox && (pointerFileDragActive || !!listDragGhost) && (
             <span className="text-sky-300/90 ml-2 font-medium">
               {listDragGhost?.copy || listDragGhost?.dropHint?.toLowerCase().includes('copy')
                 ? 'Dragging â€” drop to copy'
                 : 'Dragging â€” drop to move'}
             </span>
           )}
           {isGlobal && isGlobalSearchLoading && (
             <span className="text-amber-400/90 ml-2">Searchingâ€¦</span>
           )}
           {isGlobal && !isGlobalSearchLoading && globalSearchEngine && (
             <span className="text-amber-400/70 ml-2">
               Global Â· {globalSearchEngine === 'indexed' ? 'Indexed search'
         ...
         <div className="flex items-center gap-4 shrink-0">
            ...
            {getHoverPending() && config.showFileInfoTips !== false && config.listHoverTooltipsEnabled !== false && (
              <span className={`bndz-shift-hint hidden md:inline-flex ${isShiftKeyHeld() ? 'bndz-shift-hint-active' : ''}`}>
                <kbd>â‡§</kbd> Shift + hover for file details
              </span>
            )}
            {(() => {
              const totalCap = drives.reduce((s, d) => s + (d.totalSpace || 0), 0);
              ...
              return (
                <div className="flex items-center gap-2" title={`${formatSize(totalFree)} free of ${formatSize(totalCap)} (${pctFree}% free)`}>
                  ...
                  <span className="bndz-glass-chip text-[10px] px-2 py-0.5 font-mono text-[#c8c8c8]">
                    {drives.length} vol Â· {formatSize(totalFree)} free ({pctFree}%)
                  </span>
                </div>
              );
            })()}
      ...
      {isLoadTabsetOpen && (
        ...
        >
          {/* Settings â†’ Tabsets can revert after saving settings */}
      ...
      {isConfigDialogOpen && (
        <Suspense fallback={null}>
          <BndzErrorBoundary
            isolate
            label="Configuration"
            resetKey={configInitialTab || 'config'}
            onError={() => {
              setToastMessage('Configuration hit a render error â€” try Reload UI or reopen Config.', 'warning');
            }}
            fallback={
              <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/55">
                <div className="rounded-[14px] border border-white/12 bg-[#141820] px-6 py-5 text-center shadow-2xl max-w-md">
                  <div className="text-[14px] font-semibold text-[#e8eef6]">Configuration failed to open</div>
                  <p className="mt-2 text-[12px] text-[#9CA3AF]">A settings panel crashed. Your main UI is still running.</p>
                  <button
                    type="button"
                    className="mt-4 rounded-[10px] border border-white/15 bg-white/5 px-4 py-2 text-[12px] text-[#c5cdd6] hover:bg-white/10"
                    onClick={() => { setConfigInitialTab(undefined); setIsConfigDialogOpen(false); }}
                  >
                    Close
                  </button>
                </div>
      ...
      {contextMenu && (
        ...
        const next = setPathCacheEntry(prev, key, nextList);
          ...
          onOpenInBndz={(panePaths, opts) => {
            ...
            const win = toWindowsPath(target);
            import('../lib/ipcBridge').then(({ IPC }) => {
              IPC.recordPathOpen(win);
              IPC.executeContextMenuVerb(win, 'open');
            });
          ...
          onRestoreRecycleItems={async paths => {
            ...
            if (isQueuedIpcResult(result)) {
              setToastMessage('Restore queued â€” see transfer panel.');
              return;
            }
      ...
      <BndzErrorBoundary
        isolate
        label="Quick Look"
        resetKey={quickPreviewOpen ? (homeQuickPreview?.items?.[0]?.path || quickPreviewItems[quickPreviewIndex]?.path || 'ql') : 'ql-closed'}
        fallback={null}
        onError={() => {
          setQuickPreviewOpen(false);
          setHomeQuickPreview(null);
