...
function NativeTerminalHole({
  ...
  busy: boolean;
  /** When true, visible layout pulses clear sticky soft-park (warm session return). */
  unpark?: boolean;
}) {
  const holeRef = useRef<HTMLDivElement>(null);
  const unparkRef = useRef(unpark);
  unparkRef.current = unpark;
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      // Do NOT publish visible:false here. Effect re-runs / Strict remounts parked a live
      // session ~1s after Open and DestroyWindow raced FE remount (StackOverflow). Intentional
      // hide is publishLayout(false) when !active, plus BottomPluginPanel on tab leave.
    };
...
export default function MeshPlugin({ onNavigate, currentPath, pluginLaunch, selectedPaths, isPluginTabActive = true, popout = false }: Props) {
  ...
  useEffect(() => {
    ...
    // Pulses past the native ~120ms leave-race window + late layout settle.
    publish();
    const delays = [32, 140, 280, 480, 800];
    const timers = delays.map((ms) => window.setTimeout(publish, ms));
    return () => {
      allowUnparkRef.current = false;
          ...
          {(terminalMode || sessionId) && (
            ...
            >
              ...
              <div className="relative flex-1 min-h-0 min-w-0">
                {nativeTerm ? (
                  <NativeTerminalHole active={showNativeOverlay} sessionLabel={sessionLabel} busy={busy} unpark={!!sessionId} />
                ) : (
                  <>
                    <MeshTerminalPanel
                      sessionId={sessionId}
