  ...
  if (layout === 'grid') {
    return (
      ...
      >
        ...
        <div className="text-[12px] font-medium text-center truncate w-full bndz-drive-card-title" title={title}>
          {displayLabel}{showLetterSuffix ? <span className="bndz-drive-card-sub"> ({letter})</span> : null}
        </div>
        {!skipFree && !calculating && <StorageUsageBar usedPct={usedPct} height={6} className="w-full" />}
        <div className={`text-[10px] text-center truncate w-full bndz-drive-card-meta ${calculating ? 'text-amber-300/80 animate-pulse' : ''}`}>
          {freeOfTotal}
        </div>
      </div>
    );
      ...
      >
        <div className="bndz-list-select-cell flex items-center gap-2.5 flex-1 min-w-0">
          ...
          <div className="flex-1 min-w-0">
            <div className="text-[12px] font-medium truncate bndz-drive-card-title" title={title}>
              {displayLabel}{showLetterSuffix ? <span className="bndz-drive-card-sub"> ({letter})</span> : null}
            </div>
          </div>
        </div>
        {!skipFree && (
          <div className="flex-1 min-w-0 max-w-[45%]">
            ...
            <div className={`text-[10px] mt-1 truncate bndz-drive-card-meta ${calculating ? 'text-amber-300/80 animate-pulse' : ''}`}>
              {freeOfTotal}
            </div>
          </div>
        )}
        {skipFree && (
          <div className="text-[10px] bndz-drive-card-meta shrink-0">{freeOfTotal}</div>
        )}
      </div>
    );
  }
  ...
  if (layout === 'details') {
    return (
      <div className="bndz-drive-card flex-1 flex items-center min-w-0 gap-3">
        <div className="bndz-list-select-cell w-[30%] min-w-[110px] max-w-[280px] px-2 truncate text-[12px] bndz-drive-card-title" title={title}>
          {displayLabel}{showLetterSuffix ? <span className="bndz-drive-card-sub"> ({letter})</span> : null}
        </div>
        <div className="bndz-list-select-cell w-[14%] max-w-[110px] px-2 text-[12px] bndz-drive-card-meta truncate">{drive.type || drive.format || 'Local Disk'}</div>
        <div className="flex-1 min-w-[160px] max-w-[280px] px-2">
          {!skipFree && !calculating && <StorageUsageBar usedPct={usedPct} height={6} />}
          <div className={`text-[10px] mt-0.5 truncate bndz-drive-card-meta ${calculating ? 'text-amber-300/80 animate-pulse' : ''}`}>
            {freeOfTotal}
          </div>
        </div>
      </div>
  ...
  return (
    <div className={`bndz-drive-card px-3 py-1.5 cursor-pointer transition-colors${selClass}`}>
      <div className="flex items-center gap-2 mb-1 bndz-drive-card-title">
        ...
        {showLetterSuffix ? <span className="text-[11px] bndz-drive-card-sub truncate">({letter})</span> : null}
      </div>
