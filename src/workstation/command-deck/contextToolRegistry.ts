...
export function toolsForSignature(sig: SelectionSignature): ContextTool[] {
  ...
  if (sig.kind === 'multi') {
    const tools: ContextTool[] = [
      ...
      tool('dropstack', 'Drop Zone', 'dropstack', 'dropstack'),
      tool('ram-staging', 'Staging', 'hard_drive_ui', 'ram-staging'),
  ...
  switch (sig.media) {
    ...
    case 'folder':
      return [
        ...
        tool('library-health', 'Health', 'heart_monitor_ui', 'storage-cleanup'),
        tool('branching-time', 'Branches', 'history_ui', 'branching-time'),
        tool('ghost-link', 'Offload', 'link_ui', 'ram-staging'),
