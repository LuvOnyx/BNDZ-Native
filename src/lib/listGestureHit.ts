...
/**
 * Signal 1 — explicit marquee zones marked in the DOM (lead/trail/pad/gutters/empty canvas).
 */
 ...
 * 3) Item content → item
 * 4) Row chrome without content → marquee
 * 5) Outside any row → marquee
 * 6) Fallback inside a row → item (preserve drag)
 */
export function classifyListPointerDown(
  ...
  if (!el.closest('.fs-item-wrapper')) return 'marquee';
